...
var _delete = function deleteAtPath(data, path) {
    ...
    for(var i = 0; i < parts.length - 1; i++) {
        if(current) {
            current = canReflect_1_18_0_canReflect.getKeyValue( current, parts[i]);
        }
    }

    if(current) {
        canReflect_1_18_0_canReflect.deleteKeyValue(current, parts[parts.length - 1 ]);
    }
};
...
function get(obj, name) {
    ...
    for (i = 0; i < length && canKey_1_2_1_utils.isContainer(current) && current !== null; i++) {
        container = current;
        current = canReflect_1_18_0_canReflect.getKeyValue( container, parts[i] );
    }
...
function set$1(object, path, value) {
    ...
    // Set the value
    if (current) {
        canReflect_1_18_0_canReflect.setKeyValue(current, parts[i], value);
    } else {
...
var walk = function walk(obj, name, keyCallback){
    ...
    for (i = 0; i < length; i++) {
        container = current;
        part = parts[i];
        current = canKey_1_2_1_utils.isContainer(container) && canReflect_1_18_0_canReflect.getKeyValue( container, part );
...
function deleteKeys(parentsAndKeys) {
    for(var i  = parentsAndKeys.length - 1; i >= 0; i--) {
        ...
        if(canReflect_1_18_0_canReflect.size(parentAndKey.parent) !== 0) {
            return;
        }
    }
}
...
var transform = function(obj, transformer){
    var copy = canReflect_1_18_0_canReflect.serialize( obj);

    canReflect_1_18_0_canReflect.eachKey(transformer, function(writeKey, readKey){
...
function getDeepSize ( root, level ) {
	if ( level === 0 ) {
		return canReflect_1_18_0_canReflect.size( root );
	} else if ( canReflect_1_18_0_canReflect.size( root ) === 0 ) {
		return 0;
	} else {
		var count = 0;
		canReflect_1_18_0_canReflect.each( root, function ( value ) {
...
function getDeep ( node, items, depth, maxDepth ) {
	...
	if ( maxDepth === depth ) {
		if ( canReflect_1_18_0_canReflect.isMoreListLikeThanMapLike( node ) ) {
			canReflect_1_18_0_canReflect.addValues( items, canReflect_1_18_0_canReflect.toArray( node ) );
		} else {
			throw new Error( "can-key-tree: Map-type leaf containers are not supported yet." );
		}
	} else {
		canReflect_1_18_0_canReflect.each( node, function ( value ) {
...
function clearDeep ( node, keys, maxDepth, deleteHandler ) {
	if ( maxDepth === keys.length ) {
		if ( canReflect_1_18_0_canReflect.isMoreListLikeThanMapLike( node ) ) {
			var valuesToRemove = canReflect_1_18_0_canReflect.toArray( node );
			if(deleteHandler) {
				valuesToRemove.forEach(function(value){
					deleteHandler.apply(null, keys.concat(value));
				});
			}
			canReflect_1_18_0_canReflect.removeValues( node, valuesToRemove );
		} else {
			throw new Error( "can-key-tree: Map-type leaf containers are not supported yet." );
		}
	} else {
		canReflect_1_18_0_canReflect.each( node, function ( value, key ) {
			clearDeep( value, keys.concat(key), maxDepth, deleteHandler );
			canReflect_1_18_0_canReflect.deleteKeyValue( node, key );
		});
	}
}
...
var KeyTree = function ( treeStructure, callbacks ) {
	...
	if ( canReflect_1_18_0_canReflect.isConstructorLike( FirstConstructor ) ) {
...
// ## Methods
canReflect_1_18_0_canReflect.assign(KeyTree.prototype,{
    // ### Add
    add: function ( keys ) {
        ...
        for ( var i = 0; i < keys.length - 1; i++ ) {
    		var key = keys[i];
    		var childNode = canReflect_1_18_0_canReflect.getKeyValue( place, key );
    		if ( !childNode ) {
    			...
    			canReflect_1_18_0_canReflect.setKeyValue( place, key, childNode );
    		}
    		place = childNode;
    	}

        // Add the final leaf value in the tree.
    	if ( canReflect_1_18_0_canReflect.isMoreListLikeThanMapLike( place ) ) {
    		canReflect_1_18_0_canReflect.addValues( place, [keys[keys.length - 1]] );
    	} else {
    ...
    getNode: function ( keys ) {
        ...
        for ( var i = 0; i < keys.length; i++ ) {
            var key = keys[i];
            node = canReflect_1_18_0_canReflect.getKeyValue( node, key );
            if ( !node ) {
                return;
            }
        }
    ...
    delete: function ( keys, deleteHandler ) {
        ...
        for ( var i = 0; i < keys.length - 1; i++ ) {
    		var key = keys[i];
    		var childNode = canReflect_1_18_0_canReflect.getKeyValue( parentNode, key );
        ...
        if ( !keys.length ) {
    		...
    		if ( canReflect_1_18_0_canReflect.isMoreListLikeThanMapLike( parentNode ) ) {
				if(deleteHandler) {
					deleteHandler.apply(null, keys.concat(lastKey));
				}
    			canReflect_1_18_0_canReflect.removeValues( parentNode, [lastKey] );
    		} else {
    			throw new Error( "can-key-tree: Map types are not supported yet." );
    		}
    	}
            ...
            var nodeToRemove = canReflect_1_18_0_canReflect.getKeyValue( parentNode, lastKey );
    		if ( nodeToRemove !== undefined ) {
    			clearDeep( nodeToRemove, keys, this.treeStructure.length - 1, deleteHandler );
    			canReflect_1_18_0_canReflect.deleteKeyValue( parentNode, lastKey );
    		} else {
    			return false;
    		}
    	}
    	...
    	for ( i = path.length - 2; i >= 0; i-- ) {
    		if ( canReflect_1_18_0_canReflect.size( parentNode ) === 0 ) {
    			parentNode = path[i];
    			canReflect_1_18_0_canReflect.deleteKeyValue( parentNode, keys[i] );
    		} else {
    			break;
    		}
    	}
        // Call `onEmpty` if the tree is now empty.
    	if (  canReflect_1_18_0_canReflect.size( this.root ) === 0 ) {
			this.empty = true;
...
var mergeValueDependencies = function mergeValueDependencies(obj, source) {
	...
	if (sourceValueDeps) {
		...
		canReflect_1_18_0_canReflect.eachIndex(sourceValueDeps, function(dep) {
...
var mergeKeyDependencies = function mergeKeyDependencies(obj, source) {
	...
	if (sourcekeyDeps) {
		...
		canReflect_1_18_0_canReflect.eachKey(sourcekeyDeps, function(keys, obj) {
			...
			canReflect_1_18_0_canReflect.eachIndex(keys, function(key) {
				entry.add(key);
			});
		});
	}
	...
	"can.dispatch": function(value, old) {
		...
		if(process.env.NODE_ENV !== 'production') {
			queuesArgs = [
				...
				, null
				, [canReflect_1_18_0_canReflect.getName(this), "changed to", value, "from", old]
				/* jshint laxcomma: false */
			];
		}
		//!steal-remove-end
		canQueues_1_3_1_canQueues.enqueueByQueue.apply(canQueues_1_3_1_canQueues, queuesArgs);
		//!steal-remove-start
		if(process.env.NODE_ENV !== 'production') {
		...
		if(process.env.NODE_ENV !== 'production') {
			...
			if (notifyHandlers.length) {
				notifyHandlers.forEach(function(handler) {
					var changes = canReflect_1_18_0_canReflect.getChangesDependencyRecord(handler);
...
var mixinValueEventBindings = function(obj) {
	canReflect_1_18_0_canReflect.assign(obj, properties);
	canReflect_1_18_0_canReflect.assignSymbols(obj, symbols);
...
var unbindTemporarilyBoundValue = function () {
	for (var i = 0, len = observables.length; i < len; i++) {
		canReflect_1_18_0_canReflect.offValue(observables[i], temporarilyBoundNoOperation);
	}
	observables = null;
};
...
function temporarilyBind(compute) {
	...
	canReflect_1_18_0_canReflect.onValue(computeInstance, temporarilyBoundNoOperation);
	if (!observables) {
		observables = [];
...
function Observation(func, context, options){
	this.deriveQueue = canQueues_1_3_1_canQueues.deriveQueue;
	...
	if (process.env.NODE_ENV !== 'production') {
		...
		Object.defineProperty(this.onDependencyChange, "name", {
			value: canReflect_1_18_0_canReflect.getName(this) + ".onDependencyChange",
		});
		Object.defineProperty(this.update, "name", {
			value: canReflect_1_18_0_canReflect.getName(this) + ".update",
		});
		this._name = canReflect_1_18_0_canReflect.getName(this); // cached for performance
	}
	//!steal-remove-end
}
...
canReflect_1_18_0_canReflect.assign(Observation.prototype, {
			...
			if (process.env.NODE_ENV !== 'production') {
				queuesArgs = [
					...
					[],
					{
						...
						element: this.options.element
						/* jshint laxcomma: true */
						, log: [ canReflect_1_18_0_canReflect.getName(this.update) ]
						/* jshint laxcomma: false */
					}
					/* jshint laxcomma: true */
					, [canReflect_1_18_0_canReflect.getName(context), "changed"]
					/* jshint laxcomma: false */
				];
			}
			//!steal-remove-end
	...
	log: function() {
		...
		if (process.env.NODE_ENV !== 'production') {
			...
			this._log = function(previous, current) {
				dev.log(
					canReflect_1_18_0_canReflect.getName(this),
...
var observationProto = {
	...
	"can.setElement": function(element) {
		...
		this.deriveQueue = canQueues_1_3_1_canQueues.domQueue || canQueues_1_3_1_canQueues.deriveQueue;
	}
};
...
if (process.env.NODE_ENV !== 'production') {
	observationProto["can.getName"] = function() {
		return canReflect_1_18_0_canReflect.getName(this.constructor) + "<" + canReflect_1_18_0_canReflect.getName(this.func) + ">";
	};
}
//!steal-remove-end
canReflect_1_18_0_canReflect.assignSymbols(Observation.prototype, observationProto);
...
var addMutatedBy = function(mutatedByMap) {
	return function addMutatedBy(mutated, key, mutator) {
		...
		if (mutator.valueDependencies) {
			canReflect_1_18_0_canReflect.addValues(
		...
		if (mutator.keyDependencies) {
			canReflect_1_18_0_canReflect.each(mutator.keyDependencies, function(keysSet, obj) {
				...
				canReflect_1_18_0_canReflect.addValues(entry, keysSet);
			});
		}
	};
};
...
var deleteMutatedBy = function(mutatedByMap) {
	return function deleteMutatedBy(mutated, key, mutator) {
		...
		if (mutator.valueDependencies) {
			canReflect_1_18_0_canReflect.removeValues(
				dependencyRecord.valueDependencies,
				mutator.valueDependencies
			);
		}
		...
		if (mutator.keyDependencies) {
			canReflect_1_18_0_canReflect.each(mutator.keyDependencies, function(keysSet, obj) {
				...
				if (entry) {
					canReflect_1_18_0_canReflect.removeValues(entry, keysSet);
					if (!entry.size) {
...
var getKeyDependencies = function getKeyDependencies(obj, key) {
	if (isFunction(obj[getKeyDependenciesSymbol])) {
		return canReflect_1_18_0_canReflect.getKeyDependencies(obj, key);
	}
};

var getValueDependencies = function getValueDependencies(obj) {
	if (isFunction(obj[getValueDependenciesSymbol$1])) {
		return canReflect_1_18_0_canReflect.getValueDependencies(obj);
	}
};
...
var getWhatIChange = function getWhatIChange(obj, key) {
	if (isFunction(obj[getWhatIChangeSymbol])) {
		...
		return gotKey ?
			canReflect_1_18_0_canReflect.getWhatIChange(obj, key) :
			canReflect_1_18_0_canReflect.getWhatIChange(obj);
	}
};
...
var key = function keyObservable(root, keyPath) {
	...
	if (process.env.NODE_ENV !== 'production') {
		...
		symbolsToAssign["can.getName"] = function getName() {
			var objectName = canReflect_1_18_0_canReflect.getName(root);
	...
	return canReflect_1_18_0_canReflect.assignSymbols(observation, symbolsToAssign);
};
...
canReflect_1_18_0_canReflect.assignMap(SimpleObservable.prototype, {
...
if (process.env.NODE_ENV !== 'production') {
	simpleObservableProto["can.getName"] = function() {
		...
		return canReflect_1_18_0_canReflect.getName(this.constructor) + "<" + value$$1 + ">";
	};
}
//!steal-remove-end
...
var peek = canObservationRecorder_1_3_1_canObservationRecorder.ignore(canReflect_1_18_0_canReflect.getValue.bind(canReflect_1_18_0_canReflect));
...
function SettableObservable(fn, context, initialValue) {
	...
	//!steal-remove-start
	if (process.env.NODE_ENV !== 'production') {
		canReflect_1_18_0_canReflect.assignSymbols(this, {
			"can.getName": function() {
				return (
					canReflect_1_18_0_canReflect.getName(this.constructor) +
					"<" +
					canReflect_1_18_0_canReflect.getName(fn) +
					">"
				);
			}
		});
		Object.defineProperty(this.handler, "name", {
			value: canReflect_1_18_0_canReflect.getName(this) + ".handler"
		});
		Object.defineProperty(observe, "name", {
			value: canReflect_1_18_0_canReflect.getName(fn) + "::" + canReflect_1_18_0_canReflect.getName(this.constructor)
		});
	}
	//!steal-remove-end
...
canReflect_1_18_0_canReflect.assignMap(SettableObservable.prototype, {
	...
	handler: function(newVal) {
		...
		if (process.env.NODE_ENV !== 'production') {
			...
			reasonLog = [canReflect_1_18_0_canReflect.getName(this),"set to", newVal, "from", old];
		}
		//!steal-remove-end

		// adds callback handlers to be called w/i their respective queue.
		canQueues_1_3_1_canQueues.enqueueByQueue(
			this.handlers.getNode([]),
			this,
			[newVal, old],
			null,
	...
	activate: function(){
		canReflect_1_18_0_canReflect.onValue(this.observation, this.handler, "notify");
		this._value = peek(this.observation);
	},
	onUnbound: function() {
		this.bound = false;
		canReflect_1_18_0_canReflect.offValue(this.observation, this.handler, "notify");
	},
	set: function(newVal) {
		var oldVal =  this.lastSetValue.get();

		if (
			canReflect_1_18_0_canReflect.isObservableLike(oldVal) &&
			canReflect_1_18_0_canReflect.isValueLike(oldVal) &&
			!canReflect_1_18_0_canReflect.isObservableLike(newVal)
		) {
			canReflect_1_18_0_canReflect.setValue(oldVal, newVal);
		} else {
			if (newVal !== oldVal) {
	...
	hasDependencies: function() {
		return canReflect_1_18_0_canReflect.valueHasDependencies(this.observation);
	},
	getValueDependencies: function() {
		return canReflect_1_18_0_canReflect.getValueDependencies(this.observation);
	}
});
...
canReflect_1_18_0_canReflect.assignSymbols(SettableObservable.prototype, {
	"can.getValue": SettableObservable.prototype.get,
	"can.setValue": SettableObservable.prototype.set,
	"can.isMapLike": false,
	"can.getPriority": function() {
		return canReflect_1_18_0_canReflect.getPriority(this.observation);
	},
	"can.setPriority": function(newPriority) {
		canReflect_1_18_0_canReflect.setPriority(this.observation, newPriority);
	},
...
var canValue_1_1_2_canValue = canNamespace_1_0_0_canNamespace.value = {
	...
	from: function(object, keyPath) {
		...
		//!steal-remove-start
		if (process.env.NODE_ENV !== 'production') {
			var objectName = canReflect_1_18_0_canReflect.getName(object);
	...
	to: function(object, keyPath) {
		...
		//!steal-remove-start
		if (process.env.NODE_ENV !== 'production') {
			canReflect_1_18_0_canReflect.assignSymbols(observable.onDependencyChange, {
		...
		return canReflect_1_18_0_canReflect.assignSymbols(observable, symbolsToAssign);
	},
...
var peek$1 = canObservationRecorder_1_3_1_canObservationRecorder.ignore(canReflect_1_18_0_canReflect.getValue.bind(canReflect_1_18_0_canReflect));
	...
	if (!this.inGetter) {
		...
		if (process.env.NODE_ENV !== 'production') {
			queuesArgs = [
				...
				null
				/* jshint laxcomma: true */
				, [canReflect_1_18_0_canReflect.getName(this), "resolved with", newVal]
				/* jshint laxcomma: false */
			];
		}
		//!steal-remove-end
...
canReflect_1_18_0_canReflect.assignSymbols(Globals.prototype, {
...
function makeDelegator (domEvents) {
	...
	canReflect_1_18_0_canReflect.assignSymbols( Delegator.prototype, {
		"can.setKeyValue": function(eventType, handlersBySelector){
			var handler = this.delegated[eventType] = function(ev){
				...
				do {
					...
					canReflect_1_18_0_canReflect.each(handlersBySelector, function(handlers, selector){
...
var metaSymbol = canSymbol_1_6_5_canSymbol.for("can.meta"),
	...
	inSetupSymbol = canSymbol_1_6_5_canSymbol.for("can.initializing");
...
var ensureMeta = function ensureMeta(obj) {
	...
	if (!meta) {
		meta = {};
		canReflect_1_18_0_canReflect.setKeyValue(obj, metaSymbol, meta);
	}
...
function stopListeningArgumentsToKeys(bindTarget, event, handler, queueName) {
	if(arguments.length && canReflect_1_18_0_canReflect.isPrimitive(bindTarget)) {
		queueName = handler;
		handler = event;
		event = bindTarget;
...
var props = {
	...
	dispatch: function(event, args) {
		...
		if (this.__inSetup !== true && this[inSetupSymbol] !== true) {
			...
			if(process.env.NODE_ENV !== 'production') {
				if (!event.reasonLog) {
					event.reasonLog = [canReflect_1_18_0_canReflect.getName(this), "dispatched", '"' + event.type + '"', "with"].concat(args);
				}
			}
			...
			if ( batch ) {
				canQueues_1_3_1_canQueues.batch.start();
			}
			if(handlersByType) {
				if (handlersByType.onKeyValue) {
					canQueues_1_3_1_canQueues.enqueueByQueue(handlersByType.onKeyValue, this, args, event.makeMeta, event.reasonLog);
				}
				if (handlersByType.event) {
					event.batchNum = canQueues_1_3_1_canQueues.batch.number();
					var eventAndArgs = [event].concat(args);
					canQueues_1_3_1_canQueues.enqueueByQueue(handlersByType.event, this, eventAndArgs, event.makeMeta, event.reasonLog);
				}
			}
			if(keysNode) {
				canQueues_1_3_1_canQueues.enqueueByQueue(keysNode, this, [event.keyChanged], event.makeMeta, event.reasonLog);
			}
			if(patchesNode) {
				canQueues_1_3_1_canQueues.enqueueByQueue(patchesNode, this, [event.patches], event.makeMeta, event.reasonLog);
			}
			if(dispatchConstructorPatches) {
				this.constructor[dispatchInstanceOnPatchesSymbol](this, event.patches);
			}
			if ( batch ) {
				canQueues_1_3_1_canQueues.batch.stop();
			}
		}
		return event;
	},
	...
	listenTo: function (bindTarget, event, handler, queueName) {
		...
		if(canReflect_1_18_0_canReflect.isPrimitive(bindTarget)) {
			queueName = handler;
			handler = event;
			event = bindTarget;
			bindTarget = this;
	...
	on: function(eventName, handler, queue) {
		...
		if (listenWithDOM) {
			...
			if (this[onEventSymbol]) {
				...
				canReflect_1_18_0_canReflect.onKeyValue(this, eventName, handler, queue);
			} else {
				if (!eventName && this[onValueSymbol$1]) {
					canReflect_1_18_0_canReflect.onValue(this, handler, queue);
				} else {
	...
	off: function(eventName, handler, queue) {
		...
		if (listenWithDOM) {
			...
			if (this[offEventSymbol]) {
				...
				canReflect_1_18_0_canReflect.offKeyValue(this, eventName, handler, queue);
			} else {
				if (!eventName && this[offValueSymbol]) {
					canReflect_1_18_0_canReflect.offValue(this, handler, queue);
				} else {
...
legacyMapBindings = function(obj) {
	// add properties
	canReflect_1_18_0_canReflect.assignMap(obj, props);
	// add symbols
	return canReflect_1_18_0_canReflect.assignSymbols(obj, symbols$1);
};
...
canReflect_1_18_0_canReflect.assignMap(legacyMapBindings, props);
canReflect_1_18_0_canReflect.assignSymbols(legacyMapBindings, symbols$1);
...
defineNonEnumerable(legacyMapBindings, "flush", function() {
	...
	canQueues_1_3_1_canQueues.flush();
});

defineNonEnumerable(legacyMapBindings, "afterPreviousEvents", function(handler) {
	console.warn("don't use afterPreviousEvents");
	canQueues_1_3_1_canQueues.mutateQueue.enqueue(function afterPreviousEvents() {
		canQueues_1_3_1_canQueues.mutateQueue.enqueue(handler);
	});
	canQueues_1_3_1_canQueues.flush();
});
...
function ResolverObservable(resolver, context, initialValue, options) {
	...
	if (process.env.NODE_ENV !== 'production') {
		canReflect_1_18_0_canReflect.assignSymbols(this, {
			"can.getName": function() {
				return (
					canReflect_1_18_0_canReflect.getName(this.constructor) +
					"<" +
					canReflect_1_18_0_canReflect.getName(resolver) +
					">"
				);
			}
		});
		...
		canReflect_1_18_0_canReflect.assignSymbols(this._valueOptions.lastSet, {
			"can.getName": function() {
				return (
					canReflect_1_18_0_canReflect.getName(this.constructor)  +"::lastSet"+
					"<" +
					canReflect_1_18_0_canReflect.getName(resolver) +
					">"
				);
	...
	this[metaSymbol$1] = canReflect_1_18_0_canReflect.assignMap({}, options);
}
...
canReflect_1_18_0_canReflect.assignMap(ResolverObservable.prototype, {
		...
		if (process.env.NODE_ENV !== 'production') {
			if(!handler.name) {
				Object.defineProperty(handler, "name", {
					value:
						(bindTarget ?
							 canReflect_1_18_0_canReflect.getName(bindTarget) : "")+
	...
	resolve: function(newVal) {
		...
		if(this._value !== this.lastValue) {
			...
			if (process.env.NODE_ENV !== 'production') {
				...
				enqueueMeta = {
					log: [canReflect_1_18_0_canReflect.getName(this.update)],
					reasonLog: [canReflect_1_18_0_canReflect.getName(this), "resolved with", newVal]
				};
				/* jshint laxcomma: false */
			}
			//!steal-remove-end
			...
			canQueues_1_3_1_canQueues.batch.start();
			canQueues_1_3_1_canQueues.deriveQueue.enqueue(
				this.update,
				this,
				[],
				enqueueMeta
	...
	getValueDependencies: function getValueDependencies() {
		if (this.bound) {
			...
			if (listenHandlers) {
				canReflect_1_18_0_canReflect.each(listenHandlers.root, function(events, obj) {
					canReflect_1_18_0_canReflect.each(events, function(queues, eventName) {
...
canReflect_1_18_0_canReflect.assignSymbols(ResolverObservable.prototype, {
...
var metaSymbol$2 = canSymbol_1_6_5_canSymbol.for("can.meta");
...
function ensureMeta$1(obj) {
    ...
    if (!meta) {
        meta = {};
        canReflect_1_18_0_canReflect.setKeyValue(obj, metaSymbol$2, meta);
    }
...
function onOffAndDispatch(symbolName, dispatchName, handlersName){
    ...
    props$1["can."+dispatchName] = function(instance, arg){
        canQueues_1_3_1_canQueues.enqueueByQueue(ensureMeta$1(this)[handlersName].getNode([]), this, [instance, arg]);
    };
}
...
function mixinTypeBindings(obj){
    return canReflect_1_18_0_canReflect.assignSymbols(obj,props$1);
}
...
var canType_1_1_2_canType = createCommonjsModule(function (module, exports) {
var isMemberSymbol = canSymbol_1_6_5_canSymbol.for("can.isMember");
var newSymbol = canSymbol_1_6_5_canSymbol.for("can.new");
var getSchemaSymbol = canSymbol_1_6_5_canSymbol.for("can.getSchema");
var baseTypeSymbol = canSymbol_1_6_5_canSymbol.for("can.baseType");
var strictTypeOfSymbol = canSymbol_1_6_5_canSymbol.for("can.strictTypeOf");
...
function canNew(value) {
	...
	return canReflect_1_18_0_canReflect.convert(value, this[baseTypeSymbol]);
}
...
function check(Type, val) {
	...
	throw new Error('Type value ' + typeof val === "string" ? '"' + val + '"' : val + ' (' + valueType + ') is not of type ' + canReflect_1_18_0_canReflect.getName(Type) + '.'	);
}
...
function getBaseType(Type) {
	if(typeof Type === "function") {
		if(canReflect_1_18_0_canReflect.hasOwnKey(Type, strictTypeOfSymbol)) {
...
function makeMaybeSchema(baseType) {
	var baseSchema = canReflect_1_18_0_canReflect.getSchema(baseType);
...
function wrapName(wrapper, Type) {
	var baseName = canReflect_1_18_0_canReflect.getName(Type);
	return "type." + wrapper + "(" + baseName + ")";
}

canReflect_1_18_0_canReflect.each({
	"boolean": Boolean,
	"number": Number,
	"string": String
...
function isTypeObject(Type) {
	if(canReflect_1_18_0_canReflect.isPrimitive(Type)) {
		return false;
	}
...
function late(fn) {
	...
	return canReflect_1_18_0_canReflect.assignSymbols(lateType, {
		"can.new": function(val) {
			return canReflect_1_18_0_canReflect.new(unwrap(), val);
		},
...
var Any = canReflect_1_18_0_canReflect.assignSymbols({}, {
...
function all(typeFn, Type) {
	...
	typeObject[getSchemaSymbol] = function() {
		var parentSchema = canReflect_1_18_0_canReflect.getSchema(Type);
		var schema = canReflect_1_18_0_canReflect.assignMap({}, parentSchema);
		schema.keys = {};
		canReflect_1_18_0_canReflect.eachKey(parentSchema.keys, function(value, key) {
	...
	function Constructor(values) {
		var schema = canReflect_1_18_0_canReflect.getSchema(this);
		var keys = schema.keys;
		var convertedValues = {};
		canReflect_1_18_0_canReflect.eachKey(values || {}, function(value, key) {
			convertedValues[key] = canReflect_1_18_0_canReflect.convert(value, keys[key]);
		});
		return canReflect_1_18_0_canReflect.new(Type, convertedValues);
	}

	canReflect_1_18_0_canReflect.setName(Constructor, "Converted<" + canReflect_1_18_0_canReflect.getName(Type) + ">");
...
canReflect_1_18_0_canReflect.setName(Integer, "Integer");
...
exports.check = makeCache(function(Type) {
	...
	canReflect_1_18_0_canReflect.setName(o, wrapName("check", Type));
	return o;
});
...
exports.convert = makeCache(function(Type) {
	...
	inheritFrom(o, Type, isMemberSymbol);
	inheritFrom(o, Type, getSchemaSymbol);
	canReflect_1_18_0_canReflect.setName(o, wrapName("convert", Type));
	return o;
});
...
exports.maybe = makeCache(function(Type) {
	...
	canReflect_1_18_0_canReflect.setName(o, wrapName("maybe", Type));
	return o;
});
...
exports.maybeConvert = makeCache(function(Type) {
	...
	var o = Object.create(baseType, desc);
	canReflect_1_18_0_canReflect.setName(o, wrapName("maybeConvert", Type));
	return o;
});
...
var canType_1_1_2_canType_1 = canType_1_1_2_canType.check;
var canType_1_1_2_canType_2 = canType_1_1_2_canType.convert;
var canType_1_1_2_canType_3 = canType_1_1_2_canType.maybe;
var canType_1_1_2_canType_4 = canType_1_1_2_canType.maybeConvert;
var canType_1_1_2_canType_5 = canType_1_1_2_canType.Any;
var canType_1_1_2_canType_6 = canType_1_1_2_canType.Integer;
var canType_1_1_2_canType_7 = canType_1_1_2_canType.late;
var canType_1_1_2_canType_8 = canType_1_1_2_canType.isTypeObject;
var canType_1_1_2_canType_9 = canType_1_1_2_canType.normalize;
var canType_1_1_2_canType_10 = canType_1_1_2_canType.all;
var canType_1_1_2_canType_11 = canType_1_1_2_canType.convertAll;
...
var newSymbol$1 = Symbol.for("can.new"),
	serializeSymbol = Symbol.for("can.serialize"),
	inSetupSymbol$1 = Symbol.for("can.initializing"),
...
var peek$2 = canObservationRecorder_1_3_1_canObservationRecorder.ignore(canReflect_1_18_0_canReflect.getValue.bind(canReflect_1_18_0_canReflect));
	...
	Object_defineNamedPrototypeProperty = function(obj, prop, definition) {
		if (definition.get) {
			Object.defineProperty(definition.get, "name", {
				value: "get "+canReflect_1_18_0_canReflect.getName(obj) + "."+prop,
				writable: true,
				configurable: true
			});
		}
		if (definition.set) {
			Object.defineProperty(definition.set, "name", {
				value:  "set "+canReflect_1_18_0_canReflect.getName(obj) + "."+prop,
				configurable: true
			});
		}
...
var define_1 = define = function(typePrototype, defines, baseDefine, propertyDefaults = {}) {
	...
	canReflect_1_18_0_canReflect.eachKey(result.definitions, function(definition, property){
...
var callAsync = function(fn) {
	return function asyncResolver(lastSet, resolve){
		...
		if(canReflect_1_18_0_canReflect.isPromise(newValue)) {
...
define.property = function(typePrototype, prop, definition, dataInitializers, computedInitializers, defaultDefinition) {
	...
	if(process.env.NODE_ENV !== 'production') {
		if(!definition.set && definition.get && definition.get.length === 0 && ( "default" in definition ) ) {
			dev.warn("can-observable-object: default value for property " +
					canReflect_1_18_0_canReflect.getName(typePrototype)+"."+ prop +
	...
	if(process.env.NODE_ENV !== 'production') {
		if (definition.get) {
			Object.defineProperty(definition.get, "name", {
				value: canReflect_1_18_0_canReflect.getName(typePrototype) + "'s " + prop + " getter",
				configurable: true
			});
		}
		if (definition.set) {
			Object.defineProperty(definition.set, "name", {
				value: canReflect_1_18_0_canReflect.getName(typePrototype) + "'s " + prop + " setter",
				configurable: true
			});
		}
		if(definition.value) {
			Object.defineProperty(definition.value, "name", {
				value: canReflect_1_18_0_canReflect.getName(typePrototype) + "'s " + prop + " value",
				configurable: true
			});
		}
	}
	...
	// Determine a function that will provide the initial property value.
		...
		if (process.env.NODE_ENV !== 'production') {
			...
			if (definition.default !== null && typeof definition.default === 'object') {
				dev.warn("can-observable-object: The default value for " + canReflect_1_18_0_canReflect.getName(typePrototype)+"."+prop + " is set to an object. This will be shared by all instances of the DefineMap. Use a function that returns the object instead.");
			}
			// If value is a constructor, give a warning
			if (definition.default && canReflect_1_18_0_canReflect.isConstructorLike(definition.default)) {
				dev.warn("can-observable-object: The \"default\" for " + canReflect_1_18_0_canReflect.getName(typePrototype)+"."+prop + " is set to a constructor. Did you mean \"Default\" instead?");
			}
		}
		//!steal-remove-end
...
define.makeDefineInstanceKey = function(constructor) {
	constructor[Symbol.for("can.defineInstanceKey")] = function(property, value) {
		...
		if(value && typeof value.value !== "undefined") {
			...
			value.type = canType_1_1_2_canType.Any;
			delete value.value;
		}
...
make = {
		...
		return function(){
			...
			if(process.env.NODE_ENV !== 'production') {
				Object.defineProperty(computeObj.handler, "name", {
					value: canReflect_1_18_0_canReflect.getName(definition.value).replace('value', 'event emitter')
				});
			}
			//!steal-remove-end
			return computeObj;
	...
	compute: function(prop, get, defaultValueFn) {
		...
		return function() {
			...
			if(process.env.NODE_ENV !== 'production') {
				Object.defineProperty(computeObj.handler, "name", {
					value: canReflect_1_18_0_canReflect.getName(get).replace('getter', 'event emitter')
				});
			}
			//!steal-remove-end
	...
	set: {
		...
		computed: function(prop) {
			return function(val) {
				canReflect_1_18_0_canReflect.setValue( this._computed[prop].compute, val );
			};
		},
			...
			return function(newVal) {
				if (this[inSetupSymbol$1]) {
					...
					if (newVal !== current) {
						...
						//!steal-remove-start
						if(process.env.NODE_ENV !== 'production') {
							dispatched.reasonLog = [ canReflect_1_18_0_canReflect.getName(this) + "'s", prop, "changed to", newVal, "from", current ];
						}
						//!steal-remove-end
				...
				if (setterCalled) {
					...
					if (hasGetter) {
						...
						// we are expecting something
							...
							if(process.env.NODE_ENV !== 'production') {
								asyncTimer = setTimeout(function() {
									dev.warn('can-observable-object: Setter "' + canReflect_1_18_0_canReflect.getName(self)+"."+prop + '" did not return a value or call the setter callback.');
								}, dev.warnTimeout);
							}
							//!steal-remove-end
							canQueues_1_3_1_canQueues.batch.stop();
						...
						// we are expecting something
							...
							if(process.env.NODE_ENV !== 'production') {
								asyncTimer = setTimeout(function() {
									dev.warn('can/map/setter.js: Setter "' + canReflect_1_18_0_canReflect.getName(self)+"."+prop + '" did not return a value or call the setter callback.');
								}, dev.warnTimeout);
							}
							//!steal-remove-end
							canQueues_1_3_1_canQueues.batch.stop();
			...
			if(isDefineType(type)) {
				...
				if(type.canDefineType) {
					...
					return function setter(newValue){
						...
						if(process.env.NODE_ENV !== 'production') {
							try {
								return set.call(this, canReflect_1_18_0_canReflect.convert(newValue, type));
							} catch (error) {
								var typeName = canReflect_1_18_0_canReflect.getName(type[baseTypeSymbol]);
	...
	read: {
		...
		computed: function(prop) {
			...
			return function() {
				return canReflect_1_18_0_canReflect.getValue( this._computed[prop].compute );
			};
		},
		lastSet: function(prop) {
			return function() {
				...
				if(observable.lastSetValue) {
					return canReflect_1_18_0_canReflect.getValue(observable.lastSetValue);
				}
			};
		}
	},
	...
	get: {
		...
		computed: function(prop) {
			return function(/*val*/) {
				...
				if (canObservationRecorder_1_3_1_canObservationRecorder.isRecording()) {
					canObservationRecorder_1_3_1_canObservationRecorder.add(this, prop);
					if (!canReflect_1_18_0_canReflect.isBound(compute)) {
	...
	if(behavior === "enumerable") {
		...
		if(behavior === "async") {
			if(value.length === 1 && isAsyncFunction(value)) {
				dev.warn(`${canReflect_1_18_0_canReflect.getName(typePrototype)}: async property [${prop}] should not be an async function and also use the resolve() argument. Remove the argument and return a value from the async function instead.`);
			}
		}
...
makeDefinition = function(prop, def, defaultDefinition, typePrototype) {
	...
	canReflect_1_18_0_canReflect.eachKey(defaultDefinition, function(value, prop){
	...
	if (def.type) {
		...
		definition.type = canType_1_1_2_canType.normalize(value);
	}
	...
	if (definition.hasOwnProperty("default")) {
		if (typeof definition.default === "function" && !definition.default.isAGetter && noTypeDefined) {
			definition.type = canType_1_1_2_canType.normalize(Function);
		}

		if (canReflect_1_18_0_canReflect.isPrimitive(definition.default) && noTypeDefined) {
			definition.type = canType_1_1_2_canType.normalize(definition.default.constructor);
		}
	}

	// if there's no type definition, take it from the defaultDefinition
	if(!definition.type) {
		var defaultsCopy = canReflect_1_18_0_canReflect.assignMap({}, defaultDefinition);
		definition = canReflect_1_18_0_canReflect.assignMap(defaultsCopy, definition);
	}

	if(canReflect_1_18_0_canReflect.size(definition) === 0) {
		definition.type = canType_1_1_2_canType.Any;
...
getDefinitionOrMethod = function(prop, value, defaultDefinition, typePrototype){
    ...
    // copies a `Type`'s methods over
		...
		if(canReflect_1_18_0_canReflect.isConstructorLike(value)) {
			definition = { type: canType_1_1_2_canType.normalize(value) };
		} else {
			definition = { default: value, type: Function };
		}
	} else if( Array.isArray(value) ) {
		definition = { type: canType_1_1_2_canType.normalize(Array) };
	} else if( canReflect_1_18_0_canReflect.isPlainObject(value) ){
		definition = value;
	}
...
getDefinitionsAndMethods = function(defines, baseDefines, typePrototype, propertyDefaults) {
	...
	function addDefinition(prop, propertyDescriptor, skipGetDefinitionForMethods) {
		...
		if(prop === "constructor" || skipGetDefinitionForMethods && typeof value === "function") {
			...
			if(result && resultType === "object" && canReflect_1_18_0_canReflect.size(result) > 0) {
				...
				//!steal-remove-start
					...
					if(process.env.NODE_ENV !== 'production') {
                    	// Ex: {prop: 0}
						dev.error(canReflect_1_18_0_canReflect.getName(typePrototype)+"."+prop + " does not match a supported definitionObject. See: https://canjs.com/doc/can-observable-object/object.types.definitionObject.html");
					}
				}
				//!steal-remove-end
			}
...
function setupComputed(instance, eventName) {
	...
	if (computedBinding && computedBinding.compute) {
		if (!computedBinding.count) {
			...
			canReflect_1_18_0_canReflect.onValue(computedBinding.compute, computedBinding.handler, "notify");
...
function teardownComputed(instance, eventName){
	...
	if (computedBinding) {
		if (computedBinding.count === 1) {
			...
			canReflect_1_18_0_canReflect.offValue(computedBinding.compute, computedBinding.handler,"notify");
		} else {
			computedBinding.count--;
		}
	}
...
var onKeyValueSymbol$2 = Symbol.for("can.onKeyValue");
var offKeyValueSymbol$1 = Symbol.for("can.offKeyValue");

canReflect_1_18_0_canReflect.assignSymbols(eventsProto,{
...
define.setup = function(props, sealed) {
	...
	var map = this;
	canReflect_1_18_0_canReflect.eachKey(props, function(value, prop){
		...
		if(definitions[prop] !== undefined) {
			...
			if(sealed) {
				throw new Error(`The type ${canReflect_1_18_0_canReflect.getName(map.constructor)} is sealed, but the property [${prop}] has no definition.`);
			}

			define.expando(map, prop, value);
		}
	});
	if(canReflect_1_18_0_canReflect.size(instanceDefinitions) > 0) {
		defineConfigurableAndNotEnumerable(this, "_instanceDefinitions", instanceDefinitions);
	}
	if(requiredButNotProvided.size) {
		var msg, missingProps = Array.from(requiredButNotProvided);
		let thisName = canReflect_1_18_0_canReflect.getName(this);
...
define.normalizeTypeDefinition = canType_1_1_2_canType.normalize;
...
define.expando = function(map, prop, value) {
	...
	if(!instanceDefines) {
		if(Object.isSealed(map)) {
			let errorMessage = `Cannot set property [${prop}] on sealed instance of ${canReflect_1_18_0_canReflect.getName(map)}`;
			throw new Error(errorMessage);
		}
...
define.hooks = {
	...
	initialize: function(instance, props) {
		...
		if (!instance[canMetaSymbol].initialized) {
			defineConfigurableAndNotEnumerable(instance, inSetupSymbol$1, true);

			define.setup.call(instance, props, sealed);

			// set inSetup to false so events can be dispatched
			instance[inSetupSymbol$1] = false;
	...
	normalizeTypeDefinition: canType_1_1_2_canType.normalize //define.normalizeTypeDefinition
};
...
const defineHelpers = {
	...
	reflectSerialize: function(unwrapped){
		...
		this.forEach(function(val, name){
			...
			if(propDef && typeof propDef.serialize === "function") {
				...
				val = canReflect_1_18_0_canReflect.serialize(val);
			}
			if(val !== undefined) {
				unwrapped[name] = val;
			}
	...
	reflectUnwrap: function(unwrapped){
		this.forEach(function(value, key){
			if(value !== undefined) {
				unwrapped[key] = canReflect_1_18_0_canReflect.unwrap(value);
			}
		});
		return unwrapped;
	},
	log: function(key) {
		...
		var meta = ensureMeta$2(instance);
		...
		meta._log = function(event, data) {
			...
			if (type === "add" || type === "remove") {
				dev.log(
					canReflect_1_18_0_canReflect.getName(instance),
...
function assign(source) {
	canQueues_1_3_1_canQueues.batch.start();
	canReflect_1_18_0_canReflect.assignMap(this, source || {});
	canQueues_1_3_1_canQueues.batch.stop();
}
function update(source) {
	canQueues_1_3_1_canQueues.batch.start();
	if (canReflect_1_18_0_canReflect.isListLike(source)) {
		canReflect_1_18_0_canReflect.updateList(this, source);
	} else {
		canReflect_1_18_0_canReflect.updateMap(this, source || {});
	}
	canQueues_1_3_1_canQueues.batch.stop();
}
function assignDeep(source){
	...
	canReflect_1_18_0_canReflect.assignDeepMap(this, source || {});
	canQueues_1_3_1_canQueues.batch.stop();
}
function updateDeep(source){
	canQueues_1_3_1_canQueues.batch.start();
	if (canReflect_1_18_0_canReflect.isListLike(source)) {
		canReflect_1_18_0_canReflect.updateDeepList(this, source);
	} else {
		// TODO: we should probably just throw an error instead of cleaning
		canReflect_1_18_0_canReflect.updateDeepMap(this, source || {});
	}
	canQueues_1_3_1_canQueues.batch.stop();
}
	...
	return class extends Type {
		...
		get(prop){
			if(prop) {
				...
				return canReflect_1_18_0_canReflect.unwrap(this, Map);
			}
		}
		...
		forEach(cb, thisarg, observe) {
			function forEach(list, cb, thisarg){
				return canReflect_1_18_0_canReflect.eachKey(list, cb, thisarg);
			}
		...
		[Symbol.for("can.getOwnKeys")]() {
			var keys = canReflect_1_18_0_canReflect.getOwnEnumerableKeys(this);
			if(this._computed) {
				var computedKeys = canReflect_1_18_0_canReflect.getOwnKeys(this._computed);
...
const inSetupSymbol$2 = canSymbol_1_6_5_canSymbol.for("can.initializing");
...
function proxyPrototype(Base) {
	...
	const getHandler = isProtoReadOnSuper ?
		function(target, key, receiver) {
			if (!this[inSetupSymbol$2] && typeof key !== "symbol" && key !== "__proto__") {
				canObservationRecorder_1_3_1_canObservationRecorder.add(receiver, key);
			}
			return Reflect.get(target, key, receiver);
		} :
		function(target, key, receiver) {
			if (!this[inSetupSymbol$2] && typeof key !== "symbol") {
				canObservationRecorder_1_3_1_canObservationRecorder.add(receiver, key);
			}
			return Reflect.get(target, key, receiver);
		};
...
const helpers$1 = {
	...
	dispatchLengthPatch: function(how, patches, newLength, oldLength) {
		...
		//!steal-remove-start
		if(process.env.NODE_ENV !== "production") {
			dispatchArgs.reasonLog = [canReflect_1_18_0_canReflect.getName(this) + "." + how + " called with", arguments];
		}
		//!steal-remove-end
	...
	convertItem: function(Constructor, item) {
		if(Constructor.items) {
			...
			return canReflect_1_18_0_canReflect.convert(item, definition);
		}
		return item;
	},
...
const computedHelpers = {
	...
	addKeyDependencies: function(proxyKeys) {
		...
		canReflect_1_18_0_canReflect.assignSymbols(proxyKeys, {
			"can.onKeyValue": function(key) {
...
const { isSymbolLike: isSymbolLike$1 } = canReflect_1_18_0_canReflect;
...
canReflect_1_18_0_canReflect.eachKey(mutateMethods, function(makePatches, prop){
	...
	var mutateMethod = function() {
		...
		if (makeSideEffects === true) {
			//!steal-remove-start
			var reasonLog = [canReflect_1_18_0_canReflect.getName(meta.proxy)+"."+prop+" called with", arguments];
			//!steal-remove-end
			var dispatchArgs = {
				type: "length",
				key: "length",
...
function isListLike$1(items) {
	return canReflect_1_18_0_canReflect.isListLike(items) && typeof items !== "string";
}
...
class ObservableArray extends MixedInArray {
	...
	constructor(items, props) {
		...
		mixins$4.finalizeClass(new.target);
		mixins$4.initialize(this, props || {});

		for(let i = 0, len = items && items.length; i < len; i++) {
			this[i] = convertItem$1(new.target, items[i]);
		}
	}
	...
	splice(...args) {
		...
		for (i = 0, len = args.length - 2; i < len; i++) {
			...
			args[listIndex] = convertItem$1(this.constructor, args[listIndex]);
	...
	static convertsTo(Type) {
		const ConvertedType = canType_1_1_2_canType.convert(Type);
		...
		const name = `ConvertedObservableArray<${canReflect_1_18_0_canReflect.getName(Type)}>`;
		canReflect_1_18_0_canReflect.setName(ArrayType, name);
...
canReflect_1_18_0_canReflect.eachKey(mutateMethods$1, function(makePatches, prop) {
...
const inSetupSymbol$3 = Symbol.for("can.initializing");
...
var mixinLifecycleMethods = function mixinLifecycleMethods(BaseElement = HTMLElement) {
	return class LifecycleElement extends BaseElement {
		constructor() {
			...
			defineConfigurableNonEnumerable(this, inSetupSymbol$3, true);
		...
		initialize(props) {
			...
			this[inSetupSymbol$3] = true;

			if (super.initialize) {
				super.initialize(props);
			}

			this[inSetupSymbol$3] = false;
...
const { mixinElement: mixinElement$1 } = mixins;
		...
		if(process.env.NODE_ENV !== 'production') {
			...
			Object.keys(props).forEach(function(key) {
				if("on" + key in Type.prototype) {
					dev.warn(`${canReflect_1_18_0_canReflect.getName(Type)}: The defined property [${key}] matches the name of a DOM event. This property could update unexpectedly. Consider renaming.`);
				}
			});
		}
		//!steal-remove-end
...
var customElements$1 = canGlobals_1_2_2_canGlobalsInstance.makeExport('customElements');
...
var contains$1 = canDomMutate_2_0_7_Util.contains;
var mutate = {};
var isConnected;
...
var canDomMutate_2_0_7_IsConnected = mutate;

var eliminate$1 = canDomMutate_2_0_7_Util.eliminate;
var subscription$1 = canDomMutate_2_0_7_Util.subscription;
var isDocumentElement$1 = canDomMutate_2_0_7_Util.isDocumentElement;
var getAllNodes$1 = canDomMutate_2_0_7_Util.getAllNodes;
...
function dispatchTreeMutation(mutation, processedState) {
	...
	for (var r = 0; r < removedCount; r++) {
		...
		removedNodes.forEach(function(node){
			...
			if( canDomMutate_2_0_7_Util.wasNotInSet(node, processedState.removed) ) {
				dispatchNodeRemoved( event );
			}
			if(wasConnected && canDomMutate_2_0_7_Util.wasNotInSet(node, processedState.disconnected) ) {
	...
	for (var a = 0; a < addedCount; a++) {
		...
		insertedNodes.forEach(function(node){
			...
			if(canDomMutate_2_0_7_Util.wasNotInSet(node, processedState.inserted)) {
				dispatchNodeInserted( event );
			}
			if(wasConnected && canDomMutate_2_0_7_Util.wasNotInSet(node, processedState.connected) ) {
	...
	dispatchNodeInsertion: function (node, target) {
		queue.enqueueMutationsAndFlushAsync(
			[{
				...
				target: target,
				addedNodes: [node],
				isConnected: canDomMutate_2_0_7_IsConnected.isConnected(target),
				removedNodes: []
			}]
		);
		/*
...
var canDomMutate_2_0_7_canDomMutate = canNamespace_1_0_0_canNamespace.domMutate = domMutate;

var getParents$1 = canDomMutate_2_0_7_Util.getParents;
...
var compat = {
	replaceChild: function (newChild, oldChild) {
		...
		canDomMutate_2_0_7_canDomMutate.dispatchNodeRemoval(oldChild, this);
		for (var i = 0; i < newChildren.length; i++) {
			canDomMutate_2_0_7_canDomMutate.dispatchNodeInsertion(newChildren[i], this);
		}
		return result;
	},
	setAttribute: function (name, value) {
		...
		if (oldAttributeValue !== newAttributeValue) {
			canDomMutate_2_0_7_canDomMutate.dispatchNodeAttributeChange(this, name, oldAttributeValue);
		}
		return result;
	},
...
compatData.forEach(function (pair) {
	...
	compat[nodeMethod] = function (node) {
		...
		for (var i = 0; i < nodes.length; i++) {
			canDomMutate_2_0_7_canDomMutate[dispatchMethod](nodes[i], this);
		}
		return result;
	};
});
...
nodeMethods.forEach(function (methodName) {
	normal[methodName] = function () {
		if(canDomMutate_2_0_7_IsConnected.isConnected(this)) {
...
var node = canNamespace_1_0_0_canNamespace.domMutateNode = canDomMutate_2_0_7_canDomMutate.node = mutate$1;

// backwards compatibility
var canDomMutate_2_0_7_node = canNamespace_1_0_0_canNamespace.node = node;
...
var makeFrag = function(item, doc) {
	...
	if (!item || typeof item === "string") {
		...
		return frag;
	} else if (canReflect_1_18_0_canReflect.isListLike(item)) {
		frag = document.createDocumentFragment();
		canReflect_1_18_0_canReflect.eachIndex(item, function(item) {
...
var enableMutationObserver = function() {
	...
	var undoOnInsertionHandler = canDomMutate_2_0_7_canDomMutate.onConnected(docEl, function(mutation) {
...
var attrs = function(attrMap) {
	var map = canReflect_1_18_0_canReflect.getKeyValue(attrMap, callbackMapSymbol) || attrMap;
	...
	canReflect_1_18_0_canReflect.eachKey(map, function(callback, exp){
		attr(exp, callback);
	});
};
...
var callbacks = {
	...
	tagHandler: function(el, tagName, tagData){
		...
		if (res && tagData.subtemplate) {
			...
			canDomMutate_2_0_7_node.appendChild.call(el, frag);
		}
	}
};
			...
			if(node.attrs) {
				for(var attrName in node.attrs) {
					...
					if(typeof value === "function"){
						...
						canDomMutate_2_0_7_node.setAttribute.call(el, attrName, value);
					}
				}
			}
			if(node.attributes) {
		...
		if(keepsTextNodes) {
			...
			getCallback().callbacks.push({
				callback: function(){
					...
					canDomMutate_2_0_7_node.replaceChild.call(this.parentNode, el, this);
...
function setVirtualProp(promise, property, value) {
	...
	canQueues_1_3_1_canQueues.enqueueByQueue(observeData.handlers.getNode([property]), promise, [value,old], function() {
		return {};
...
function setupPromise(value) {
	...
	canReflect_1_18_0_canReflect.assignSymbols(proto, {
...
var peek$3 = canObservationRecorder_1_3_1_canObservationRecorder.ignore(canReflect_1_18_0_canReflect.getKeyValue.bind(canReflect_1_18_0_canReflect));
var observeReader;
...
if (process.env.NODE_ENV !== 'production') {
	bindName = function(source){
		...
		Object.defineProperty(fn, "name", {
			value: canReflect_1_18_0_canReflect.getName(source) + "."+canReflect_1_18_0_canReflect.getName(this)
		});
		return fn;
	};
}
...
var objHasKeyAtIndex = function(obj, reads, index) {
	return !!(
		reads && reads.length &&
		canReflect_1_18_0_canReflect.hasKey(obj, reads[index].key)
	);
};
	...
	valueReaders: [
		{
			...
			test: function(value){
				return value && canReflect_1_18_0_canReflect.isFunctionLike(value) && !canReflect_1_18_0_canReflect.isConstructorLike(value);
			},
			read: function(value, i, reads, options, state, prev){
				if(options.callMethodsOnObservables && canReflect_1_18_0_canReflect.isObservableLike(prev) && canReflect_1_18_0_canReflect.isMapLike(prev)) {
		...
		},
		{
			...
			read: function(value, i, reads, options){
				...
				return canReflect_1_18_0_canReflect.getValue(value);
			},
	...
	propertyReaders: [
		{
			...
			test: function(value){
				...
				//  special reflections.
				if(canReflect_1_18_0_canReflect.isPromise(value) ||
					isPromiseLike(value)) {
					canReflectPromise_2_2_1_canReflectPromise(value);
				}

				return canReflect_1_18_0_canReflect.isObservableLike(value) && canReflect_1_18_0_canReflect.isMapLike(value);
			},
			read: function(value, prop){
				var res = canReflect_1_18_0_canReflect.getKeyValue(value, prop.key);
				if(res !== undefined) {
					return res;
				} else {
			...
			write: canReflect_1_18_0_canReflect.setKeyValue
		},
		...
		// read a normal object
		{
			...
			write: function(base, prop, newVal){
				...
				if(newVal != null && typeof newVal === "object" && canReflect_1_18_0_canReflect.isMapLike(propValue) ) {
					dev.warn("can-stache-key: Merging data into \"" + prop + "\" because its parent is non-observable");
					canReflect_1_18_0_canReflect.update(propValue, newVal);
				} else if(propValue != null && propValue[setValueSymbol$2] !== undefined){
					canReflect_1_18_0_canReflect.setValue(propValue, newVal);
				} else {
					base[prop] = newVal;
				}
			}
...
var inSetupSymbol$4 = canSymbol_1_6_5_canSymbol.for("can.initializing");
 ...
 * @static
 */
canReflect_1_18_0_canReflect.assignMap(Construct, {
	/**
	...
	newInstance: function () {
		...
		if (inst.setup) {
			...
			Object.defineProperty(inst, inSetupSymbol$4, {
				configurable: true,
				enumerable: false,
				value: true,
				writable: true
	 ...
	 */
	setup: function (base) {
		var defaults = base.defaults ? canReflect_1_18_0_canReflect.serialize(base.defaults) : {};
		this.defaults = canReflect_1_18_0_canReflect.assignDeepMap(defaults,this.defaults);
	},
	...
	extend: function (name, staticProperties, instanceProperties) {
		...
		canReflect_1_18_0_canReflect.assignMap(Constructor, {
...
var SimpleMap = canConstruct_3_5_6_canConstruct.extend("SimpleMap",
	{
		...
		attr: function(prop, value) {
			...
			if(arguments.length === 0 ) {
				canObservationRecorder_1_3_1_canObservationRecorder.add(this,"can.keys");
				var data = {};
				canReflect_1_18_0_canReflect.eachKey(this._data, function(value, prop){
					canObservationRecorder_1_3_1_canObservationRecorder.add(this, prop);
					data[prop] = value;
				}, this);
				return data;
				...
				if(old !== value) {
					...
					if (process.env.NODE_ENV !== 'production') {
						dispatched = {
							keyChanged: !had ? prop : undefined,
							type: prop,
							reasonLog: [ canReflect_1_18_0_canReflect.getName(this) + "'s", prop, "changed to", value, "from", old ],
						};
					}
					//!steal-remove-end
			...
			// 1 argument
				...
				canQueues_1_3_1_canQueues.batch.start();
				canReflect_1_18_0_canReflect.eachKey(prop, function(value, key) {
					self.attr(key, value);
				});
				canQueues_1_3_1_canQueues.batch.stop();
			}
		...
		if( this._data.hasOwnProperty(prop) ) {
			...
			if (process.env.NODE_ENV !== 'production') {
				dispatched = {
					keyChanged: prop,
					type: prop,
					reasonLog: [ canReflect_1_18_0_canReflect.getName(this) + "'s", prop, "deleted", old ]
				};
			}
			//!steal-remove-end
...
if (process.env.NODE_ENV !== 'production') {
	simpleMapProto["can.getName"] = function() {
		return canReflect_1_18_0_canReflect.getName(this.constructor) + "{}";
	};
}
//!steal-remove-end
canReflect_1_18_0_canReflect.assignSymbols(SimpleMap.prototype,simpleMapProto);
...
var canViewScope_4_13_6_templateContext = TemplateContext;
...
var Compute = function(newVal){
	if(arguments.length) {
		return canReflect_1_18_0_canReflect.setValue(this, newVal);
	} else {
		return canReflect_1_18_0_canReflect.getValue(this);
	}
};

var canViewScope_4_13_6_makeComputeLike = function(observable) {
    var compute = Compute.bind(observable);

	//!steal-remove-start
	if (process.env.NODE_ENV !== 'production') {
		Object.defineProperty(compute, "name", {
			value: "Compute<"+canReflect_1_18_0_canReflect.getName(observable) + ">",
		});
	}
	//!steal-remove-end
    ...
    canReflect_1_18_0_canReflect.assignSymbols(compute, {
        "can.getValue": function(){
            return canReflect_1_18_0_canReflect.getValue(observable);
        },
        "can.setValue": function(newVal){
            return canReflect_1_18_0_canReflect.setValue(observable, newVal);
        },
        "can.onValue": function(handler, queue){
            return canReflect_1_18_0_canReflect.onValue(observable, handler, queue);
        },
        "can.offValue": function(handler, queue){
            return canReflect_1_18_0_canReflect.offValue(observable, handler, queue);
        },
        "can.valueHasDependencies": function(){
            return canReflect_1_18_0_canReflect.valueHasDependencies(observable);
        },
        "can.getPriority": function(){
    		return canReflect_1_18_0_canReflect.getPriority( observable );
    	},
    	"can.setPriority": function(newPriority){
    		canReflect_1_18_0_canReflect.setPriority( observable, newPriority );
    	},
		"can.isValueLike": true,
...
var setElementSymbol = canSymbol_1_6_5_canSymbol.for("can.setElement");
...
var getFastPathRoot = canObservationRecorder_1_3_1_canObservationRecorder.ignore(function(computeData){
	if( computeData.reads &&
		...
		if( root && root[canSymbol_1_6_5_canSymbol.for("can.getValue")] ) {
			root = canReflect_1_18_0_canReflect.getValue(root);
		}
		// on a map
		return root && canReflect_1_18_0_canReflect.isObservableLike(root) && canReflect_1_18_0_canReflect.isMapLike(root) &&
...
function getMutated(scopeKeyData){
	...
	return !canReflect_1_18_0_canReflect.isPrimitive(value$$1) ? value$$1 : scopeKeyData.root;
}
...
var ScopeKeyData = function(scope, key, options){
	...
	if (process.env.NODE_ENV !== 'production') {
		Object.defineProperty(this.read, "name", {
			value: canReflect_1_18_0_canReflect.getName(this) + ".read",
		});
		Object.defineProperty(this.dispatch, "name", {
			value: canReflect_1_18_0_canReflect.getName(this) + ".dispatch",
		});
	}
	//!steal-remove-end
...
canAssign_1_3_3_canAssign(ScopeKeyData.prototype, {
	...
	onBound: function onBound(){
		this.bound = true;
		canReflect_1_18_0_canReflect.onValue(this.observation, this.dispatch, "notify");
	...
	onUnbound: function onUnbound() {
		this.bound = false;
		canReflect_1_18_0_canReflect.offValue(this.observation, this.dispatch, "notify");
		this.toSlowPath();
	},
	set: function(newVal){
		...
		if(root) {
			if(this.reads.length) {
				...
				canReflect_1_18_0_canReflect.setValue(root,newVal);
			}
		} else {
...
if (process.env.NODE_ENV !== 'production') {
	scopeKeyDataPrototype["can.getName"] = function() {
		return canReflect_1_18_0_canReflect.getName(this.constructor) + "{{" + this.key + "}}";
	};
}
//!steal-remove-end
canReflect_1_18_0_canReflect.assignSymbols(ScopeKeyData.prototype, scopeKeyDataPrototype);

// Creates a compute-like for legacy reasons ...
Object.defineProperty(ScopeKeyData.prototype, "compute", {
	get: function(){
		var compute = canViewScope_4_13_6_makeComputeLike(this);
...
var canViewScope_4_13_6_scopeKeyData = ScopeKeyData;

var canViewScope_4_13_6_compute_data = function(scope, key, options){
	return new canViewScope_4_13_6_scopeKeyData(scope, key, options || {
		args: []
	});
};
...
var canViewScope_4_13_6_letContext = LetContext;
...
canAssign_1_3_3_canAssign(Scope, {
	...
	TemplateContext: canViewScope_4_13_6_templateContext,
	// ### keyInfo(key)
	...
	isTemplateContextOrCanNotHaveProperties: function(currentScope){
		...
		if(currentContext instanceof canViewScope_4_13_6_templateContext) {
			return true;
...
canAssign_1_3_3_canAssign(Scope.prototype, {
			...
			readOptions = canAssign_1_3_3_canAssign({
				...
				earlyExit: function(parentValue, nameIndex) {
					...
					if(isVariableScope === true && nameIndex === 0) {
						...
						updateSetObservable = canReflect_1_18_0_canReflect.hasKey( parentValue, keyReads[nameIndex].key);
					} else {
						updateSetObservable =
							// Has more matches
							nameIndex > setObserveDepth ||
							// The same number of matches but it has the key
							nameIndex === setObserveDepth && (typeof parentValue === "object" && canReflect_1_18_0_canReflect.hasOwnKey( parentValue, keyReads[nameIndex].key));
					}
	...
	getDataForScopeSet: function getDataForScopeSet(key, options) {
		...
		var opts = canAssign_1_3_3_canAssign({
			...
			read: function(context, keys){
				...
				if(firstSearchedContext === undefined && !(context instanceof canViewScope_4_13_6_letContext)) {
				...
				if(keys.length > 1) {
					...
					if( parent != null && canReflect_1_18_0_canReflect.hasKey(parent, keys[keys.length-1].key ) ) {
						return {
							parent: parent,
							parentHasKey: true,
							value: undefined
				...
				// If we have only one key, try to find a context with this key
					...
					if( canReflect_1_18_0_canReflect.hasKey(context, keys[0].key ) ) {
						return {
							parent: context,
							parentHasKey: true,
							value: undefined
		...
		if(!canReflect_1_18_0_canReflect.isObservableLike(parent) && canReflect_1_18_0_canReflect.isObservableLike(parent[propName])) {
			if(canReflect_1_18_0_canReflect.isMapLike(parent[propName])) {
				return {
					parent: parent,
					key: propName,
					how: "updateDeep",
			...
			else if(canReflect_1_18_0_canReflect.isValueLike(parent[propName])){
	...
	getHelperOrPartial: function(keyReads) {
		...
		while (scope) {
			...
			if (context instanceof canViewScope_4_13_6_templateContext) {
	...
	getTemplateContext: function() {
		...
		var templateContext = this.getScope(function(scope) {
			lastScope = scope;
			return scope._context instanceof canViewScope_4_13_6_templateContext;
		});

		// if there is no reference scope, add one as the root
		if(!templateContext) {
			templateContext = new Scope(new canViewScope_4_13_6_templateContext());
	...
	addTemplateContext: function(){
		return this.add(new canViewScope_4_13_6_templateContext());
	},
	addLetContext: function(values){
		return this.add(new canViewScope_4_13_6_letContext(values || {}), {variable: true});
	},
	// ### scope.getRoot
	...
	getRoot: function() {
		...
		if (cur._context instanceof canViewScope_4_13_6_templateContext) {
			cur = child;
		}
		return cur._context;
	},
	...
	getPathsForKey: function getPathsForKey(key) {
		//!steal-remove-start
			...
			var getKeyDefinition = function(obj, key) {
				...
				var objHasKey = canReflect_1_18_0_canReflect.hasKey(obj, key);
	...
	set: function(key, value, options) {
		...
		switch (data.how) {
			...
			case "setValue":
				canReflect_1_18_0_canReflect.setValue("key" in data ? parent[data.key] : parent, value);
				break;

			case "setKeyValue":
				canReflect_1_18_0_canReflect.setKeyValue(parent, data.key, value);
				break;

			case "updateDeep":
				canReflect_1_18_0_canReflect.updateDeep(parent[data.key], value);
				break;
		}
	},
	...
	computeData: function(key, options) {
		return canViewScope_4_13_6_compute_data(this, key, options);
	},
	...
	isSpecial: function(){
		return this._meta.notContext || this._meta.special || (this._context instanceof canViewScope_4_13_6_templateContext) || this._meta.variable;
	}
});
// Legacy name for _walk.
Scope.prototype._read = Scope.prototype._walk;

canReflect_1_18_0_canReflect.assignSymbols(Scope.prototype, {
...
if (process.env.NODE_ENV !== 'production') {
	Scope.prototype.log = function() {
		...
		while(scope) {
			...
			console.log(indent, canReflect_1_18_0_canReflect.getName(scope._context) + contextType, scope._context);
	        scope = scope._parent;
	        indent += " ";
	    }
	};
...
canNamespace_1_0_0_canNamespace.view = canNamespace_1_0_0_canNamespace.view || {};
var canViewScope_4_13_6_canViewScope = canNamespace_1_0_0_canNamespace.view.Scope = Scope;
...
var utils$1 = {
	...
	makeRendererConvertScopes: function (renderer, parentScope, observeObservables, metadata) {
		var convertedRenderer = function (newScope, newOptions) {
			...
			if (newScope !== undefined && !(newScope instanceof canViewScope_4_13_6_canViewScope)) {
				if (parentScope) {
					newScope = parentScope.add(newScope);
				}
				else {
					newScope = new canViewScope_4_13_6_canViewScope(newScope || {});
				}
			}
			if (metadata) {
				metadata.rendered = true;
	...
	makeView: function(renderer){
		var view = canObservationRecorder_1_3_1_canObservationRecorder.ignore(function(scope){
			if(!(scope instanceof canViewScope_4_13_6_canViewScope)) {
				scope = new canViewScope_4_13_6_canViewScope(scope);
			}
	...
	getItemsStringContent: function(items, isObserveList, helperOptions){
		var txt = "",
			...
			isObservable = canReflect_1_18_0_canReflect.isObservableLike(items);
	...
	getItemsFragContent: function(items, helperOptions, scope) {
		...
		// Check if using hash
		if (canReflect_1_18_0_canReflect.size(hashExprs) > 0) {
			hashOptions = {};
			canReflect_1_18_0_canReflect.eachKey(hashExprs, function (exprs, key) {
		...
		for (var i = 0; i < len; i++) {
			...
			if (canReflect_1_18_0_canReflect.size(hashOptions) > 0) {
...
function makeIdentityFromMapSchema(typeSchema) {
    if(typeSchema.identity && typeSchema.identity.length) {
        return function identityCheck(a, b) {
            var aId = canReflect_1_18_0_canReflect.getIdentity(a, typeSchema),
                bId = canReflect_1_18_0_canReflect.getIdentity(b, typeSchema);
            return aId === bId;
        };
    } else {
...
function makeIdentityFromListSchema(listSchema) {
    return listSchema.values != null ?
        makeIdentityFromMapSchema( canReflect_1_18_0_canReflect.getSchema(listSchema.values) ) :
        defaultIdentity;
}

function makeIdentity(oldList, oldListLength) {
    var listSchema = canReflect_1_18_0_canReflect.getSchema(oldList),
        typeSchema;
    if(listSchema != null) {
        if(listSchema.values != null) {
            typeSchema = canReflect_1_18_0_canReflect.getSchema(listSchema.values);
        } else {
            return defaultIdentity;
        }
    }
    if(typeSchema == null && oldListLength > 0) {
        typeSchema = canReflect_1_18_0_canReflect.getSchema( canReflect_1_18_0_canReflect.getKeyValue(oldList, 0) );
    }
    if(typeSchema) {
...
var list = function(oldList, newList, schemaOrIdentity){
    var oldIndex = 0,
		newIndex =  0,
		oldLength = canReflect_1_18_0_canReflect.size( oldList ),
		newLength = canReflect_1_18_0_canReflect.size( newList ),
		patches = [];
		...
		return {
			...
			set: function(value){
				if(prop in this) {
					...
					canDomMutate_2_0_7_node.setAttribute.call(this, prop, "");
				}
			},
			remove: function(){
				this[prop] = false;
...
var specialAttributes = {
	...
	"class": {
		...
		set: function(val){
			...
			if(isSVG(this)) {
				canDomMutate_2_0_7_node.setAttribute.call(this, "class", "" + val);
			} else {
				this.className = val;
			}
		}
		...
		set: function(val){
			...
			if (cur !== val) {
				if (!docEl.contains(element)) {
					var connectionDisposal = canDomMutate_2_0_7_canDomMutate.onNodeConnected(element, function () {
						connectionDisposal();
						focusTask();
					});
				} else {
	...
	style: {
		set: (function () {
			...
			if ( el && el.style && ("cssText" in el.style) ) {
				...
				return function (val) {
					canDomMutate_2_0_7_node.setAttribute.call(this, "style", val);
				};
			}
		})()
	},
		...
		set: function(value){
			...
			if(nodeName === "select") {
				...
				if(!docEl.contains(this)) {
					var select = this;
					var connectionDisposal = canDomMutate_2_0_7_canDomMutate.onNodeConnected(select, function () {
						connectionDisposal();
...
var attr = {
		...
		return {
			...
			set: function(val) {
				if (attrsNamespacesURI[attrName]) {
					canDomMutate_2_0_7_node.setAttributeNS.call(this, attrsNamespacesURI[attrName], attrName, val);
				} else {
					canDomMutate_2_0_7_node.setAttribute.call(this, attrName, val);
				}
			}
		};
	},
	...
	remove: function (el, attrName) {
		...
		if(typeof remover === "function" && test.call(el)) {
			...
			canDomMutate_2_0_7_node.removeAttribute.call(el, attrName);
		}
	}
};

var canAttributeObservable_2_0_1_behaviors = attr;

var setElementSymbol$1 = canSymbol_1_6_5_canSymbol.for("can.setElement");
var elementSymbol = canSymbol_1_6_5_canSymbol.for("can.element");
...
function ListenUntilRemovedAndInitialize(observable, handler, placeholder, queueName, handlerName, afterTeardownNodeRemoved) {
	...
	if(process.env.NODE_ENV !== 'production') {
		...
		canReflect_1_18_0_canReflect.assignSymbols(handler, {
		...
		canReflectDependencies_1_1_2_canReflectDependencies.addMutatedBy(placeholder, observable);
	}
	//!steal-remove-end

	this.teardownNodeRemoved = canDomMutate_2_0_7_canDomMutate.onNodeRemoved(placeholder,
		this.teardown.bind(this));

	canReflect_1_18_0_canReflect.onValue(observable, handler, queueName);

	// data = live.listen(parentNode, compute, liveHTMLUpdateHTML);
	handler(  canReflect_1_18_0_canReflect.getValue(observable) );
}
ListenUntilRemovedAndInitialize.prototype.teardown = function(){
	...
	//!steal-remove-end
	canReflect_1_18_0_canReflect.offValue(this.observable, this.handler, this.queueName);
};
	...
	range: {
		...
		remove: function ( range ) {
			...
			while(cur !== range.start) {
				...
				canDomMutate_2_0_7_node.removeChild.call(parentNode, remove );
			}

			canDomMutate_2_0_7_canDomMutate.flushRecords();
		},

		update: function ( range, frag ) {
			var parentNode = range.start.parentNode;

			canDomMutate_2_0_7_node.insertBefore.call(parentNode, frag, range.end);
...
var attr_1 = function(el, attributeName, compute) {
	...
	if(process.env.NODE_ENV !== 'production') {
		// register that the handler changes the parent element
		handlerName = "live.attr update::"+canReflect_1_18_0_canReflect.getName(compute);
	}
	//!steal-remove-end

	new helpers$2.ListenUntilRemovedAndInitialize(compute,
			function liveUpdateAttr(newVal) {
				canAttributeObservable_2_0_1_behaviors.set(el,attributeName, newVal);
			},
			el,
			"dom",
			handlerName
...
var attrs = function(el, compute, scope, options) {
	var handlerName = "";
	if (!canReflect_1_18_0_canReflect.isObservableLike(compute)) {
		...
		for (var name in attrs) {
			canDomMutate_2_0_7_node.setAttribute.call(el, name, attrs[name]);
		}
		return;
	}
	...
	//!steal-remove-start
	if(process.env.NODE_ENV !== 'production') {
		handlerName = "live.attrs update::"+canReflect_1_18_0_canReflect.getName(compute);
	}
	//!steal-remove-end
		...
		function canViewLive_updateAttributes(newVal) {
			...
			for (name in newAttrs) {
				...
				if (newValue !== oldValue) {
					...
					canDomMutate_2_0_7_node.setAttribute.call(el, name, newValue);
			...
			for (name in oldAttrs) {
				canDomMutate_2_0_7_node.removeAttribute.call(el, name);
			}
			oldAttrs = newAttrs;
		},
		el,
...
var html = function(el, compute, viewInsertSymbolOptions) {
	...
	if(process.env.NODE_ENV !== 'production') {
		...
		observableName = canReflect_1_18_0_canReflect.getName(compute);
	...
	var doNotUpdateRange = function() {
		canQueues_1_3_1_canQueues.domQueue.dequeue(updateRange);
	};
	...
	new helpers$2.ListenUntilRemovedAndInitialize(compute,
		function canViewLive_updateHTML(val) {
			...
			if(useQueue === true) {
				...
				canQueues_1_3_1_canQueues.domQueue.enqueue(updateRange, null, [range, frag], meta);
			} else {
...
var Patcher = function(observableOrList, priority) {
	...
	this.isObservableValue = canReflect_1_18_0_canReflect.isValueLike(this.observableOrList) || canReflect_1_18_0_canReflect.isObservableLike(this.observableOrList);
	if(this.isObservableValue) {
	    this.priority = canReflect_1_18_0_canReflect.getPriority(observableOrList);
	} else {
	...
	if(process.env.NODE_ENV !== 'production') {
		Object.defineProperty(this.onList, "name", {
			value: "live.list new list::"+canReflect_1_18_0_canReflect.getName(observableOrList),
		});
		Object.defineProperty(this.onPatchesNotify, "name", {
			value: "live.list notify::"+canReflect_1_18_0_canReflect.getName(observableOrList),
		});
		Object.defineProperty(this.onPatchesDerive, "name", {
			value: "live.list derive::"+canReflect_1_18_0_canReflect.getName(observableOrList),
		});
	}
	//!steal-remove-end
};
...
Patcher.prototype = {
	...
	setup: function() {
		if (this.observableOrList[onValueSymbol$2]) {
			...
			// new list.
			canReflect_1_18_0_canReflect.onValue(this.observableOrList, this.onList, "notify");
			// listen on the current value (which shoudl be a list) if there is one
			this.setupList(canReflect_1_18_0_canReflect.getValue(this.observableOrList));
		} else {
	...
	teardown: function() {
		if (this.observableOrList[offValueSymbol$1]) {
			canReflect_1_18_0_canReflect.offValue(this.observableOrList, this.onList, "notify");
		}
	...
	onPatchesNotify: function onPatchesNotify(patches) {
		...
		// TODO: share priority
		canQueues_1_3_1_canQueues.deriveQueue.enqueue(this.onPatchesDerive, this, [], {
	...
	onPatchesDerive: function onPatchesDerive() {
		...
		this.patches = [];
		canQueues_1_3_1_canQueues.enqueueByQueue(this.handlers.getNode([]), this.currentList, [patches, this.currentList], null,["Apply patches", patches]);
	}
};

canReflect_1_18_0_canReflect.assignSymbols(Patcher.prototype, {
	"can.onPatches": function(handler, queue) {
		this.handlers.add([queue || "mutate", handler]);
	},
	"can.offPatches": function(handler, queue) {
...
canReflect_1_18_0_canReflect.assignSymbols(SetObservable.prototype, {
...
function ListDOMPatcher(el, compute, render, context, falseyRender) {
	...
	var observableName = canReflect_1_18_0_canReflect.getName(compute);
	...
	this.isValueLike = canReflect_1_18_0_canReflect.isValueLike(this.value);
	this.isObservableLike = canReflect_1_18_0_canReflect.isObservableLike(this.value);
	...
	if(process.env.NODE_ENV !== 'production') {
		Object.defineProperty(this.onPatches, "name", {
			value: "live.list update::"+canReflect_1_18_0_canReflect.getName(compute),
		});
	}
	//!steal-remove-end
...
ListDOMPatcher.prototype = {
	setupValueBinding: function() {
		...
		this.teardownNodeRemoved = canDomMutate_2_0_7_canDomMutate.onNodeRemoved(this.range.start, this.teardownValueBinding);
	...
	addInDomQueue: function(items, index) {
		canQueues_1_3_1_canQueues.domQueue.enqueue(this.add, this, [items, index], this.meta);
	},
	add: function(items, index) {
		...
		items.forEach( function(item, key) {
			...
			var itemIndex = new canSimpleObservable_2_5_0_canSimpleObservable(key + index),
				itemCompute = new setObservable(item, function(newVal) {
					canReflect_1_18_0_canReflect.setKeyValue(this.patcher.currentList, itemIndex.get(), newVal );
				}.bind(this)),
		...
		canDomMutate_2_0_7_node.insertBefore.call(placeholder.parentNode,frag,placeholder);
...
var text = function(el, compute) {
	...
	if(process.env.NODE_ENV !== 'production') {
		...
		handlerName = "live.text update::"+canReflect_1_18_0_canReflect.getName(compute);
	}
	//!steal-remove-end
...
var canViewLive_5_0_0_canViewLive = live;
...
canAssign_1_3_3_canAssign(TextSectionBuilder.prototype,{
	...
	compile: function(state){
		...
		return function(scope){
			...
			canReflect_1_18_0_canReflect.onValue(observation, noop);

			var value = canReflect_1_18_0_canReflect.getValue(observation);
			if( canReflect_1_18_0_canReflect.valueHasDependencies( observation ) ) {
				if(state.textContentOnly) {
					canViewLive_5_0_0_canViewLive.text(this, observation);
				}
				else if(state.attr) {
					canViewLive_5_0_0_canViewLive.attr(this, state.attr, observation);
				}
				else {
					canViewLive_5_0_0_canViewLive.attrs(this, observation, scope);
				}
				canReflect_1_18_0_canReflect.offValue(observation, noop);
			} else {
				if(state.textContentOnly) {
					this.nodeValue = value;
				}
				else if(state.attr) {
					canDomMutate_2_0_7_node.setAttribute.call(this, state.attr, value);
				}
				else {
					canViewLive_5_0_0_canViewLive.attrs(this, value);
				}
			}
		};
	}
...
var setElementSymbol$2 = canSymbol_1_6_5_canSymbol.for("can.setElement");
		...
		canReflect_1_18_0_canReflect.assignSymbols(this, {
			"can.getName": function() {
				return (
					canReflect_1_18_0_canReflect.getName(this.constructor) +
					"<" +
					canReflect_1_18_0_canReflect.getName(getter) +
					">"
				);
			}
		});
...
canReflect_1_18_0_canReflect.assignSymbols(SetterObservable.prototype, {
	...
	"can.setElement": function(el) {
		this.observation[setElementSymbol$2](el);
	}
});
...
function getObservableValue_fromDynamicKey_fromObservable(key, root, helperOptions, readOptions) {
	...
	var getKeys = function(){
		return canStacheKey_1_4_3_canStacheKey.reads(("" + canReflect_1_18_0_canReflect.getValue(key)).replace(/\./g, "\\."));
	};
	var parentHasKey;
	var computeValue = new setter(function getDynamicKey() {
		var readData = canStacheKey_1_4_3_canStacheKey.read( canReflect_1_18_0_canReflect.getValue(root) , getKeys());
		parentHasKey = readData.parentHasKey;
		return readData.value;
	}, function setDynamicKey(newVal){
		canStacheKey_1_4_3_canStacheKey.write(canReflect_1_18_0_canReflect.getValue(root), getKeys(), newVal);
	});
...
function toComputeOrValue(value) {
	...
	if(canReflect_1_18_0_canReflect.isObservableLike(value)) {
		// we only want to do this for things that `should` have dependencies, but dont.
		if(canReflect_1_18_0_canReflect.isValueLike(value) && canReflect_1_18_0_canReflect.valueHasDependencies(value) === false) {
			return canReflect_1_18_0_canReflect.getValue(value);
		}
		// if compute data
		if(value.compute) {
			...
			return canViewScope_4_13_6_makeComputeLike(value);
		}
	}
	return value;
}
...
Hashes.prototype.value = function(scope, helperOptions){
	...
	return new canObservation_4_2_0_canObservation(function(){
		...
		for(var prop in hash) {
			finalHash[prop] = hash[prop].call ? canReflect_1_18_0_canReflect.getValue( hash[prop].value ) : expressionHelpers.toComputeOrValue( hash[prop].value );
		}
		return finalHash;
	});
};
...
if (process.env.NODE_ENV !== 'production') {
	Hashes.prototype.sourceText = function(){
		var hashes = [];
		canReflect_1_18_0_canReflect.eachKey(this.hashExprs, function(expr, prop){
		...
		for(var i = 0, len = args.length; i < len; i++) {
			if (doNotWrapArguments) {
				...
				finalArgs[i] = args[i].call ?
					canReflect_1_18_0_canReflect.getValue( args[i].value ) :
...
Call.prototype.value = function(scope, helperOptions){
	...
	var func = canReflect_1_18_0_canReflect.getValue( method );
	...
	var computeFn = function(newVal){
		var func = canReflect_1_18_0_canReflect.getValue( method );
		if(typeof func === "function") {
			if (canReflect_1_18_0_canReflect.isObservableLike(func)) {
				func = canReflect_1_18_0_canReflect.getValue(func);
			}
			var args = getArgs(
				func.isLiveBound
			);
			...
			if(func[isViewSymbol$1] === true) {
				...
				if(!(args[0] instanceof canViewScope_4_13_6_canViewScope)){
	...
	if (typeof initialValue === "function") {
		...
		if (process.env.NODE_ENV !== 'production') {
			Object.defineProperty(helperFn, "name", {
				configurable: true,
				value: canReflect_1_18_0_canReflect.getName(this)
			});
		}
		//!steal-remove-end
	}
...
if (process.env.NODE_ENV !== 'production') {
	Helper.prototype.sourceText = function(){
		...
		if(canReflect_1_18_0_canReflect.size(this.hashExprs) > 0){
			text.push( hashes.prototype.sourceText.call(this) );
		}
		return text.join(" ");
	};

	canReflect_1_18_0_canReflect.assignSymbols(Helper.prototype,{
		"can.getName": function() {
			return canReflect_1_18_0_canReflect.getName(this.constructor) + "{{" + (this.sourceText()) + "}}";
		}
	});
}
//!steal-remove-end
...
var Lookup = function(key, root, sourceText) {
	this.key = key;
	this.rootExpr = root;
	canReflect_1_18_0_canReflect.setKeyValue(this, sourceTextSymbol$1, sourceText);
};
...
var convertToAtLookup = function(ast){
	if(ast.type === "Lookup") {
		canReflect_1_18_0_canReflect.setKeyValue(ast, sourceTextSymbol$2, ast.key);
		...
		while(cursor.index < tokens.length) {
			...
			// Bracket
				...
				if (lastToken && (lastToken.type === "Call" || lastToken.type === "Bracket"  )  ) {
					...
					//!steal-remove-start
					if (process.env.NODE_ENV !== 'production') {
						canReflect_1_18_0_canReflect.setKeyValue(bracket$$1, canSymbol_1_6_5_canSymbol.for("can-stache.originalKey"), top.key);
					}
					//!steal-remove-end
...
var core = {
	...
	makeEvaluator: function (scope, mode, exprData, truthyRenderer, falseyRenderer, stringOnly) {
		...
		if(!mode || helperOptions.metadata.rendered) {
			...
			return function(){
				// Get the value
				var finalValue = canReflect_1_18_0_canReflect.getValue(value);
				var result;
				...
				else if ( typeof finalValue !== "string" && canReflect_1_18_0_canReflect.isListLike(finalValue) ) {
					var isObserveList = canReflect_1_18_0_canReflect.isObservableLike(finalValue) &&
						canReflect_1_18_0_canReflect.isListLike(finalValue);

					if(canReflect_1_18_0_canReflect.getKeyValue(finalValue, "length")) {
						if (stringOnly) {
		...
		return function(scope){
			...
			var partialFrag = new canObservation_4_2_0_canObservation(function(){
				...
				if(exprData && exprData.argExprs.length === 1) {
					var newContext = canReflect_1_18_0_canReflect.getValue( exprData.argExprs[0].value(scope) );
				...
				var partial = canReflect_1_18_0_canReflect.getKeyValue(partialScope.templateContext.partials, localPartialName);
				var renderer;
			...
			canViewLive_5_0_0_canViewLive.html(this, partialFrag);
		};
	},
	...
	makeStringBranchRenderer: function(mode, expressionString, state){
		...
		var branchRenderer = function branchRenderer(scope, truthyRenderer, falseyRenderer){
			...
			if(gotObservableValue) {
				res = canReflect_1_18_0_canReflect.getValue(evaluator);
			} else {
				res = evaluator();
			}
	...
	makeLiveBindingBranchRenderer: function(mode, expressionString, state){
		...
		var branchRenderer = function branchRenderer(scope, truthyRenderer, falseyRenderer){
			...
			canReflect_1_18_0_canReflect.onValue(observable, k);

			var value = canReflect_1_18_0_canReflect.getValue(observable);
			...
			else if( canReflect_1_18_0_canReflect.valueHasDependencies(observable) ) {
				// Depending on where the template is, setup live-binding differently.
				if(state.attr) {
					canViewLive_5_0_0_canViewLive.attr(this, state.attr, observable);
				}
				else if( state.tag )  {
					canViewLive_5_0_0_canViewLive.attrs( this, observable );
				}
					...
					//!steal-remove-end
					canViewLive_5_0_0_canViewLive.text(this, observable);
				} else {
					canViewLive_5_0_0_canViewLive.html(this, observable);
				}
			}
			// If the computeValue has no observable dependencies, just set the value on the element.
				...
				if(state.attr) {
					canDomMutate_2_0_7_canDomMutate.setAttribute(this, state.attr, value);
				}
				else if(state.tag) {
					canViewLive_5_0_0_canViewLive.attrs(this, value);
				}
			...
			// Unbind the compute.
			canReflect_1_18_0_canReflect.offValue(observable, k);
		};
...
//!steal-remove-start
if (process.env.NODE_ENV !== 'production') {
	var canReflect = canReflect_1_18_0_canReflect;
...
var truthyObservable = function(observable){
    return new canObservation_4_2_0_canObservation(function truthyObservation(){
        var val = canReflect_1_18_0_canReflect.getValue(observable);
...
function makeConverter(getterSetter){
	...
	return function(newVal, source) {
		var args = canReflect_1_18_0_canReflect.toArray(arguments);
...
var bindAndRead = function (value) {
	if ( value && canReflect_1_18_0_canReflect.isValueLike(value) ) {
...
function forOfObject(object, variableName, options){
	var result = [];
	canReflect_1_18_0_canReflect.each(object, function(val, key){
...
var forHelper = function(helperOptions) {
	...
	if(resolved && !canReflect_1_18_0_canReflect.isListLike(resolved)) {
		return forOfObject(resolved,variableName, helperOptions);
	}
	if(options.stringOnly) {
		var parts = [];
		canReflect_1_18_0_canReflect.eachIndex(resolved, function(value, index){
			var variableScope = {};
			if(variableName !== undefined){
				variableScope[variableName] = value;
			}
		...
		return function(el){
			...
			canViewLive_5_0_0_canViewLive.list(el, items, cb, options.context, function(list){
...
var letHelper = canObservationRecorder_1_3_1_canObservationRecorder.ignore(function(options){
	...
	canReflect_1_18_0_canReflect.assignMap(variableScope._context, options.hash);
...
function portalHelper(elementObservable, options){
	var debugName = "portal(" + canReflect_1_18_0_canReflect.getName(elementObservable) + ")";
	...
	function teardownPortalledContent() {
		...
		if(portalElement) {
			canReflect_1_18_0_canReflect.offValue(elementObservable, getElementAndRender);
			portalElement = null;
		}
		...
		if(startPortalledPlaceholder && endPortalledPlaceholder) {
			...
			if(parentNode) {
				...
				canDomMutate_2_0_7_node.removeChild.call(parentNode, startPortalledPlaceholder );
				canDomMutate_2_0_7_node.removeChild.call(parentNode, endPortalledPlaceholder );
	...
	function getElementAndRender() {
		...
		canReflect_1_18_0_canReflect.onValue(elementObservable, getElementAndRender);

		portalElement = canReflect_1_18_0_canReflect.getValue(elementObservable);
		...
		if(portalElement) {
			...
			canViewLive_5_0_0_canViewLive.html(startPortalledPlaceholder, observable);
		} else {
			options.metadata.rendered = true;
		}
	...
	return function(placeholderElement) {
		...
		commentPlaceholderDispose = canDomMutate_2_0_7_canDomMutate.onNodeRemoved(commentPlaceholder, teardownEverything);
...
var helpersCore = {
	...
	makeSimpleHelper: function(fn) {
		return function() {
			var realArgs = [];
			canReflect_1_18_0_canReflect.eachIndex(arguments, function(val) {
	...
	addConverter: function(name, getterSetter) {
		if(typeof name === "object") {
			if(!converterPackages.has(name)) {
				...
				canReflect_1_18_0_canReflect.eachKey(name, function(getterSetter, name) {
	...
	addBuiltInHelpers: function() {
		canReflect_1_18_0_canReflect.each(builtInHelpers, function(helper, helperName) {
			...
			if (process.env.NODE_ENV !== 'production') {
				Object.defineProperty(callLogic, "name", {
					value: name+"("+args.map(function(arg){
						return canReflect_1_18_0_canReflect.getName(arg);
					}).join(",")+")",
					configurable: true
				});
			}
...
var ifHelper = canAssign_1_3_3_canAssign(function ifHelper(expr, options) {
	...
	if (expr && canReflect_1_18_0_canReflect.isValueLike(expr)) {
		value = canReflect_1_18_0_canReflect.getValue(new truthyObservable(expr));
	} else {
...
var switchHelper = function(expression, options){
	...
	canReflect_1_18_0_canReflect.assignSymbols(defaultHelper, {
		"can.isValueLike": true,
		"can.isFunctionLike": false,
		"can.getValue": function() {
...
var joinBaseHelper = function(firstExpr/* , expr... */){
	...
	var templateModule = canReflect_1_18_0_canReflect.getKeyValue(options.scope.templateContext.helpers, 'module');
...
var eachHelper = function(items) {
	...
	if ((
		canReflect_1_18_0_canReflect.isObservableLike(resolved) && canReflect_1_18_0_canReflect.isListLike(resolved) ||
			( canReflect_1_18_0_canReflect.isListLike(resolved) && canReflect_1_18_0_canReflect.isValueLike(items) )
	) && !options.stringOnly) {
		// Tells that a helper has been called, this function should be returned through
		// checking its value.
		options.metadata.rendered = true;
	...
	if (!!expr && canReflect_1_18_0_canReflect.isListLike(expr)) {
		result = utils$1.getItemsFragContent(expr, options, options.scope);
		return options.stringOnly ? result.join('') : result;
	} else if (canReflect_1_18_0_canReflect.isObservableLike(expr) && canReflect_1_18_0_canReflect.isMapLike(expr) || expr instanceof Object) {
		result = [];
		canReflect_1_18_0_canReflect.each(expr, function(val, key){
...
var withHelper = function (expr, options) {
	...
	if(!options) {
		...
		if(options.hash && canReflect_1_18_0_canReflect.size(options.hash) > 0) {
...
var notConverter = {
	get: function(obs, options){
		if(helpersCore.looksLikeOptions(options)) {
			return canReflect_1_18_0_canReflect.getValue(obs) ? options.inverse() : options.fn();
		} else {
			return !canReflect_1_18_0_canReflect.getValue(obs);
		}
	},
	set: function(newVal, obs){
		canReflect_1_18_0_canReflect.setValue(obs, !newVal);
	}
};
...
function stache (filename, template) {
	...
	var scopifiedRenderer = canObservationRecorder_1_3_1_canObservationRecorder.ignore(function(scope, options){
		...
		// helperOptions (.fn, .inverse, etc) when called as Call Expressions
		canReflect_1_18_0_canReflect.eachKey(options && options.helpers, function(helperValue) {
			helperValue.requiresOptionsArgument = true;
		});

		// helpers, partials, tags, vars
		var templateContext = new canViewScope_4_13_6_templateContext(options);

		// copy inline partials over
		canReflect_1_18_0_canReflect.eachKey(inlinePartials, function(partial, partialName) {
			canReflect_1_18_0_canReflect.setKeyValue(templateContext.partials, partialName, partial);
		});

		// allow the current renderer to be called with {{>scope.view}}
		canReflect_1_18_0_canReflect.setKeyValue(templateContext, 'view', scopifiedRenderer);
		//!steal-remove-start
		if (process.env.NODE_ENV !== 'production') {
			canReflect_1_18_0_canReflect.setKeyValue(templateContext, 'filename', section.filename);
		}
		//!steal-remove-end
		...
		if ( !(scope instanceof canViewScope_4_13_6_canViewScope) ) {
			scope = new canViewScope_4_13_6_canViewScope(templateContext).add(scope);
		} else {
			// we are going to split ...
			var templateContextScope = new canViewScope_4_13_6_canViewScope(templateContext);
...
stache.safeString = function(text){
	...
	return canReflect_1_18_0_canReflect.assignSymbols({},{
		"can.toDOM": function(){
			return text;
		}
	});
...
var canStache_5_0_0_canStache = canNamespace_1_0_0_canNamespace.stache = stache;
...
var canElementSymbol$1 = canSymbol_1_6_5_canSymbol.for("can.element");

// Default implementations for setting the child and parent values
function defaultSetValue(newValue, observable) {
	canReflect_1_18_0_canReflect.setValue(observable, newValue);
}
...
function turnOffListeningAndUpdate(listenToObservable, updateObservable, updateFunction, queue) {
	...
	if (listenToObservable[onValueSymbol$3]) {
		offValueOrOffEmitFn = canReflect_1_18_0_canReflect.offValue;
...
function turnOnListeningAndUpdate(listenToObservable, updateObservable, updateFunction, queue) {
	...
	if (listenToObservable[onValueSymbol$3]) {
		onValueOrOnEmitFn = canReflect_1_18_0_canReflect.onValue;
	} else if (listenToObservable[onEmitSymbol]) {
	...
	increment: function(args) {
		...
		if(process.env.NODE_ENV !== 'production') {
			...
			var semaphoreData = {
				...
				value: this.value,
				lastTask: canQueues_1_3_1_canQueues.lastTask()
			};
...
function Bind(options) {
	...
	if (options.priority !== undefined) {
		canReflect_1_18_0_canReflect.setPriority(options.child, options.priority);
		canReflect_1_18_0_canReflect.setPriority(options.parent, options.priority);
	}
	...
	if(process.env.NODE_ENV !== 'production') {
		...
		Object.defineProperty(this._updateChild, "name", {
			value: options.updateChildName ? options.updateChildName : "update "+canReflect_1_18_0_canReflect.getName(options.child),
			configurable: true
		});

		Object.defineProperty(this._updateParent, "name", {
			value: options.updateParentName ? options.updateParentName : "update "+canReflect_1_18_0_canReflect.getName(options.parent),
			configurable: true
		});
	}
	//!steal-remove-end
...
Object.defineProperty(Bind.prototype, "parentValue", {
	get: function() {
		return canReflect_1_18_0_canReflect.getValue(this._options.parent);
	}
});
...
canAssign_1_3_3_canAssign(Bind.prototype, {
	...
	start: function() {
		...
		if (this._childToParent === true && this._parentToChild === true) {
			// Two-way binding
			parentValue = canReflect_1_18_0_canReflect.getValue(options.parent);
			if (parentValue === undefined) {
				childValue = canReflect_1_18_0_canReflect.getValue(options.child);
			...
			if(process.env.NODE_ENV !== 'production'){
				...
				if (options.sticky && childValue !== parentValue) {
					canLog.warn(
						...
						(options.debugName || (canReflect_1_18_0_canReflect.getName(parentContext) + "<->" + canReflect_1_18_0_canReflect.getName(childContext))) +
...
function updateValue(args) {
	...
	if ((semaphore.value + args.partnerSemaphore.value) <= args.allowedUpdates) {
		...
		canQueues_1_3_1_canQueues.mutateQueue.enqueue(semaphore.decrement, semaphore, []);
		...
		if (args.sticky) {
			var observableValue = canReflect_1_18_0_canReflect.getValue(args.observable);
			if (observableValue !== canReflect_1_18_0_canReflect.getValue(args.partner)) {
		...
		//!steal-remove-start
		if(process.env.NODE_ENV !== 'production'){
			var currentValue = canReflect_1_18_0_canReflect.getValue(args.observable);
			if (currentValue !== args.newValue) {
				var warningParts = [
					"can-bind: attempting to update " + args.debugObservableName + " " + canReflect_1_18_0_canReflect.getName(args.observable) + " to new value: %o",
				...
				if(console.groupCollapsed) {
					...
					this._debugSemaphores.forEach(function(semaphoreMutation){
						if(semaphoreMutation.action === "increment") {
							console.groupCollapsed(semaphoreMutation.type+" "+canReflect_1_18_0_canReflect.getName(semaphoreMutation.observable)+" set.");
							var stack = canQueues_1_3_1_canQueues.stack(semaphoreMutation.lastTask);
							var printStack = getFromLastStack(stack);
							lastStack = stack;
							// This steals how `logStack` logs information.
							canQueues_1_3_1_canQueues.logStack.call({
								stack: function(){
									return printStack;
								}
							});
							console.log(semaphoreMutation.type+ " semaphore incremented to "+semaphoreMutation.value+".");
							console.log(canReflect_1_18_0_canReflect.getName(semaphoreMutation.observable),semaphoreMutation.observable,"set to ", semaphoreMutation.newValue);
							console.groupEnd();
						}
					});
					console.groupCollapsed(args.debugObservableName+" "+canReflect_1_18_0_canReflect.getName(args.observable)+" NOT set.");
					var stack = getFromLastStack(canQueues_1_3_1_canQueues.stack());
					canQueues_1_3_1_canQueues.logStack.call({
						stack: function(){
							return stack;
						}
					...
					console.log(args.debugObservableName+" semaphore ("+semaphore.value+
					 ...
					 args.allowedUpdates+")");
					console.log("Prevented from setting "+canReflect_1_18_0_canReflect.getName(args.observable), args.observable, "to", args.newValue);
					console.groupEnd();
				}
			}
		}
...
var canViewModel_4_0_3_canViewModel = canNamespace_1_0_0_canNamespace.viewModel = function (el, attr, val) {
	...
	} else if (canReflect_1_18_0_canReflect.isListLike(el) && !el.nodeType) {
		el = el[0];
	}

	if (canReflect_1_18_0_canReflect.isObservableLike(attr) && canReflect_1_18_0_canReflect.isMapLike(attr)) {
	...
	switch (arguments.length) {
		...
		case 1:
			return scope;
		case 2:
			return canReflect_1_18_0_canReflect.getKeyValue(scope, attr);
		default:
			canReflect_1_18_0_canReflect.setKeyValue(scope, attr, val);
			return el;
	}
};
...
var canAttributeObservable_2_0_1_event = canEvent;
...
var canAttributeObservable_2_0_1_getEventName = function getEventName(el, prop) {
	var event = "change";
	...
	if (canAttributeObservable_2_0_1_behaviors.findSpecialListener(prop)) {
		event = prop;
	}
...
function AttributeObservable(el, prop, bindingData, event) {
	...
	this.event = event || canAttributeObservable_2_0_1_getEventName(el, prop);
	this.handler = this.handler.bind(this);
...
canAssign_1_3_3_canAssign(AttributeObservable.prototype, {
	...
	get: function get() {
		...
		var value = canAttributeObservable_2_0_1_behaviors.get(this.el, this.prop);
		if (typeof value === 'function') {
	...
	set: function set(newVal) {
		var setterDispatchedEvents = canAttributeObservable_2_0_1_behaviors.setAttrOrProp(this.el, this.prop, newVal);
	...
	handler: function handler(newVal, event) {
		var old = this._value;
		var queuesArgs = [];
		this._value = canAttributeObservable_2_0_1_behaviors.get(this.el, this.prop);
	...
	onBound: function onBound() {
		...
		observable._handler = function(event) {
			observable.handler(canAttributeObservable_2_0_1_behaviors.get(observable.el, observable.prop), event);
		};

		if (observable.event === internalRadioChangeEventType) {
			canAttributeObservable_2_0_1_event.on.call(observable.el, "change", observable._handler);
		}

		var specialBinding = canAttributeObservable_2_0_1_behaviors.findSpecialListener(observable.prop);
		if (specialBinding) {
			observable._specialDisposal = specialBinding.call(observable.el, observable.prop, observable._handler, canUtilAEL);
		}

		canAttributeObservable_2_0_1_event.on.call(observable.el, observable.event, observable._handler);
	...
	onUnbound: function onUnbound() {
		...
		if (observable.event === internalRadioChangeEventType) {
			canAttributeObservable_2_0_1_event.off.call(observable.el, "change", observable._handler);
		}
		...
		canAttributeObservable_2_0_1_event.off.call(observable.el, observable.event, observable._handler);
	},
...
canReflect_1_18_0_canReflect.assignSymbols(AttributeObservable.prototype, {
	"can.isMapLike": false,
...
var canAttributeObservable_2_0_1_canAttributeObservable = AttributeObservable;
...
var onMatchStr = "on:",
	...
	viewModelSymbol$1 = canSymbol_1_6_5_canSymbol.for("can.viewModel"),
	preventDataBindingsSymbol = canSymbol_1_6_5_canSymbol.for("can.preventDataBindings");
...
var getEventBindingData = function (attributeName, el, scope) {
	...
	if (elUsed) {
		...
		if (byUsed) {
			...
			bindingContextObservable = new canViewScope_4_13_6_canViewScope(viewModel).computeData(bindingContextKey);
		}
	} else if (scopeUsed) {
...
var onKeyValueSymbol$5 = canSymbol_1_6_5_canSymbol.for("can.onKeyValue");
...
var runEventCallback = function (el, ev, data, scope, expr, attributeName, attrVal) {
	...
	var updateFn = function() {
		...
		value = canReflect_1_18_0_canReflect.isValueLike(value) ?
			canReflect_1_18_0_canReflect.getValue(value) :
			value;
	...
	//!steal-remove-end
	canQueues_1_3_1_canQueues.mutateQueue.enqueue.apply(canQueues_1_3_1_canQueues.mutateQueue, mutateQueueArgs);
	canQueues_1_3_1_canQueues.batch.stop();
};
...
var behaviors = {
	...
	initializeViewModel: function(bindings, initialViewModelData, makeViewModel, bindingContext) {
		...
		bindings.forEach(function(dataBinding){
			...
			if (siblingBindingData.parent.exports) {
				...
				var parentValue = siblingBindingData.child.setCompute ? canViewScope_4_13_6_makeComputeLike(dataBinding.binding.parent) : dataBinding.binding.parentValue;
	...
	viewModel: function(el, tagData, makeViewModel, initialViewModelData, staticDataBindingsOnly) {
		...
		canReflect_1_18_0_canReflect.eachListLike(el.attributes || [], function(node) {
		...
		if (!bindingsState.isSettingViewModel) {
			...
			attributeDisposal = canDomMutate_2_0_7_canDomMutate.onNodeAttributeChange(el, function(ev) {
	...
	data: function(el, attrData) {
		...
		attributeDisposal = canDomMutate_2_0_7_canDomMutate.onNodeAttributeChange(el, attributeListener);
		removedDisposal = canDomMutate_2_0_7_canDomMutate.onNodeDisconnected(el, function() {
	...
	event: function(el, data) {
		...
		var handler = function(ev) {
			...
			if (expr instanceof expression_1.Hashes) {
				...
				var isObservableValue = canReflect_1_18_0_canReflect.isObservableLike(value) && canReflect_1_18_0_canReflect.isValueLike(value);
				runScope.set(key, isObservableValue ? canReflect_1_18_0_canReflect.getValue(value) : value);
		...
		attributesDisposal = canDomMutate_2_0_7_canDomMutate.onNodeAttributeChange(el, attributesHandler);
		removalDisposal = canDomMutate_2_0_7_canDomMutate.onNodeDisconnected(el, removalHandler);
		if (!bindingContext && bindingContextObservable) {
			...
			removeObservation = function () {
				...
				canReflect_1_18_0_canReflect.offValue(bindingContextObservable, updateListener);
			};
			canReflect_1_18_0_canReflect.onValue(bindingContextObservable, updateListener);
		} else {
...
var getObservableFrom = {
			...
			if (mustBeGettable || scopeProp.indexOf("(") >= 0 || scopeProp.indexOf("=") >= 0) {
				...
				if (parentExpression instanceof expression_1.Hashes) {
					return new canSimpleObservable_2_5_0_canSimpleObservable(function () {
						...
						var isObservableValue = canReflect_1_18_0_canReflect.isObservableLike(value) && canReflect_1_18_0_canReflect.isValueLike(value);
						scope.set(key, isObservableValue ? canReflect_1_18_0_canReflect.getValue(value) : value);
					});
				} else {
				...
				var observation = {};
				canReflect_1_18_0_canReflect.assignSymbols(observation, {
					...
					"can.setValue": function setValue(newVal) {
						...
						var value = expr.value(scope);
						canReflect_1_18_0_canReflect.setValue(value, newVal);
					},
						...
						if (process.env.NODE_ENV !== 'production') {
							...
							if (data.parent && data.key) {
								result = "ObservableFromScope<" +
									canReflect_1_18_0_canReflect.getName(data.parent) +
									"." +
									data.key +
									">";
							}
		...
		var observation = new setter(
			...
			function setViewModelProperty(newVal) {
				...
				if (setCompute) {
					...
					var oldValue = canReflect_1_18_0_canReflect.getKeyValue(viewModel, setName);
					if (canReflect_1_18_0_canReflect.isObservableLike(oldValue)) {
						canReflect_1_18_0_canReflect.setValue(oldValue, newVal);
					} else {
						canReflect_1_18_0_canReflect.setKeyValue(
							viewModel,
							setName,
							new canSimpleObservable_2_5_0_canSimpleObservable(canReflect_1_18_0_canReflect.getValue(newVal))
						);
					}
				} else {
					if (isBoundToContext) {
						canReflect_1_18_0_canReflect.setValue(viewModel, newVal);
					} else {
	...
	attribute: function(bindingData, bindingContext ) {
		...
		if(bindingData.name === "this") {
			...
			return new canAttributeObservable_2_0_1_canAttributeObservable(bindingContext.element, bindingData.name, {}, bindingData.event);
		}
...
canReflect_1_18_0_canReflect.eachKey(siblingBindingRules, function(value, key) {
...
var canStacheBindings_5_0_1_canStacheBindings = canStacheBindings;

const rendererSymbol = Symbol.for("can.stacheRenderer");
const viewInsertSymbol$2 = Symbol.for("can.viewInsert");

// make bindings work
canStache_5_0_0_canStache.addBindings(canStacheBindings_5_0_1_canStacheBindings);
	...
	class StacheClass extends Base {
		render(props, renderOptions) {
			...
			if (!renderer) {
				...
				const viewName = canReflect_1_18_0_canReflect.getName(this.constructor) + "View";

				renderer = typeof view === "function" ?
					view :
					canStache_5_0_0_canStache(viewName, view || "");

				this.constructor[rendererSymbol] = renderer;
			}

			const frag = renderer(
				new canViewScope_4_13_6_canViewScope(this, null, { viewModel: true }),
				renderOptions
			);

			const viewRoot = this.viewRoot || this;
			canDomMutate_2_0_7_node.appendChild.call(viewRoot, frag);
		}
		...
		connect() {
			...
			const removedDisposal = canDomMutate_2_0_7_canDomMutate.onNodeRemoved(this, () => {
...
var mixinBindings = function mixinBindings(Base = HTMLElement) {
	return class BindingsClass extends Base {
		bindings(bindings) {
			if(this[metaSymbol$6] === undefined) {
				this[metaSymbol$6] = {};
			}
			const bindingsObservables = {};
			canReflect_1_18_0_canReflect.eachKey(bindings, (parent, propName) => {
			...
			this[metaSymbol$6]._connectedBindings = bindingsObservables;
			return this;
		}
		initialize(props) {
			var savedBindings = this[metaSymbol$6] && this[metaSymbol$6]._connectedBindings;
			if (savedBindings) {
				props = props || {};

				if (this[metaSymbol$6]._bindings === undefined) {
					this[metaSymbol$6]._bindings = [];
				}

				canReflect_1_18_0_canReflect.eachKey(savedBindings, (binding, propName) => {
					...
					if (canGetParentValue || canSetParentValue) {
						...
						var canBinding = new canBind_1_5_1_canBind({
							...
							updateParentName: "update " + canReflect_1_18_0_canReflect.getName(parent) + " from <" + this.nodeName.toLowerCase() + ">."+propName
							//!steal-remove-end
						});

						this[metaSymbol$6]._bindings.push({
							binding: canBinding,
							siblingBindingData: {
								parent: {
									source: "scope",
		...
		disconnect() {
			delete this[metaSymbol$6]._bindings;
			if (super.disconnect) {
				super.disconnect();
			}
		}
...
var mixinInitializeBindings = function mixinBindings(Base = HTMLElement) {
	return class InitializeBindingsClass extends Base {
		initialize(props) {
			var bindings = this[metaSymbol$7] && this[metaSymbol$7]._bindings;
			...
			if (bindings && bindings.length) {
				...
				var initializeData = canStacheBindings_5_0_1_canStacheBindings.behaviors.initializeViewModel(bindings, props, (properties) => {
					super.initialize(properties);
					return this;
				}, bindingContext);
	
				this[metaSymbol$7]._connectedBindingsTeardown = function() {
		...
		disconnect() {
			if(this[metaSymbol$7] && this[metaSymbol$7]._connectedBindingsTeardown) {
				this[metaSymbol$7]._connectedBindingsTeardown();
				this[metaSymbol$7]._connectedBindingsTeardown = null;
			}
			if (super.disconnect) {
				super.disconnect();
			}
...
var mixinBindBehaviour = function mixinBindBehaviour(Base = HTMLElement) {
	class BindingPropsClass extends Base {
		initialize(props) {
			if(this[metaSymbol$8] === undefined) {
				this[metaSymbol$8] = {};
			}
			if (this[metaSymbol$8]._bindings === undefined) {
				this[metaSymbol$8]._bindings = [];
			}
			// `_uninitializedBindings` are being set within `observedAttributes` which creates the bindings
			Object.keys(this.constructor[metaSymbol$8]._uninitializedBindings).forEach(propName => {
				const binding = this.constructor[metaSymbol$8]._uninitializedBindings[propName](this);

				// Add bindings to the instance `metaSymbol` to be set up during `mixin-initialize-bindings`
				this[metaSymbol$8]._bindings.push({
					binding,
					siblingBindingData: {
						parent: {
							source: "scope",
...
var initializeObservedAttributes = function initializeObservedAttributes (ctr) {
	Object.defineProperty(ctr, 'observedAttributes', {
		get () {
			...
			mixins$5.finalizeClass(this);
			
			if(this[metaSymbol$8] === undefined) {
				this[metaSymbol$8] = {};
			}
			if(this[metaSymbol$8]._uninitializedBindings === undefined) {
				this[metaSymbol$8]._uninitializedBindings = {};
			}
			...
			if (definitions) {
				...
				Object.keys(definitions).forEach(propName => {
					...
					if (typeof definition.bind === 'function') {
						...
						this[metaSymbol$8]._uninitializedBindings[propName] = bindFn;
...
function rendererWasCalledWithData(scope) {
	return scope instanceof canViewScope_4_13_6_canViewScope &&
		scope._parent &&
		scope._parent._context instanceof canViewScope_4_13_6_canViewScope.TemplateContext;
}
...
function DeriveElement(BaseElement = HTMLElement) {
	...
	mixinLifecycleMethods(
		...
		[initializeSymbol](el, tagData) {
			...
			const teardownBindings = canStacheBindings_5_0_1_canStacheBindings.behaviors.viewModel(
				el,
				tagData,
...
var map$2 = function(oldObject, newObject){
	...
	oldObjectClone = canReflect_1_18_0_canReflect.assignMap({}, oldObject);

    canReflect_1_18_0_canReflect.eachKey(newObject, function(value, newProp){
...
function makeDiffListIdentityComparison(oldList, newList, parentKey, nestedPatches) {
    var listSchema = canReflect_1_18_0_canReflect.getSchema(oldList),
        ...
        oldListLength = canReflect_1_18_0_canReflect.size( oldList );
    if(listSchema != null) {
        if(listSchema.values != null) {
            typeSchema = canReflect_1_18_0_canReflect.getSchema(listSchema.values);
        }
    ...
    return function(a, b, aIndex) {
        if(canReflect_1_18_0_canReflect.isPrimitive(a)) {
            return a === b;
        }
        if(canReflect_1_18_0_canReflect.isPrimitive(b)) {
            return a === b;
        }
...
function diffDeep(dest, source, parentKey){
    ...
    if (dest && canReflect_1_18_0_canReflect.isMoreListLikeThanMapLike(dest)) {
        var nestedPatches = [],
        ...
        patches.forEach(function(patch){
            ...
            var destVal = dest && canReflect_1_18_0_canReflect.getKeyValue(dest, key),
                sourceVal = source && canReflect_1_18_0_canReflect.getKeyValue(source, key);
...
function smartMerge(instance, props) {
	...
	props = canReflect_1_18_0_canReflect.serialize(props);

	if (canReflect_1_18_0_canReflect.isMoreListLikeThanMapLike(instance)) {
...
function mergeMap(instance, data) {
	...
	// for each key in
	canReflect_1_18_0_canReflect.eachKey(instance, function(value, prop) {
		if(!canReflect_1_18_0_canReflect.hasKey(data, prop)) {
			canReflect_1_18_0_canReflect.deleteKeyValue(instance, prop);
			return;
		}
		var newValue = canReflect_1_18_0_canReflect.getKeyValue(data, prop);
		canReflect_1_18_0_canReflect.deleteKeyValue(data, prop);
		...
		if (canReflect_1_18_0_canReflect.isPrimitive(value)) {
			canReflect_1_18_0_canReflect.setKeyValue(instance, prop, newValue);
			return;
		}


		var newValueIsList = Array.isArray(newValue),
			currentValueIsList = canReflect_1_18_0_canReflect.isMoreListLikeThanMapLike(value);

		if (currentValueIsList && newValueIsList) {

			mergeList(value, newValue);

		} else if (!newValueIsList && !currentValueIsList && canReflect_1_18_0_canReflect.isMapLike(value) && canReflect_1_18_0_canReflect.isPlainObject(newValue)) {

			// TODO: the `TYPE` should probably be infered from the `_define` property definition.
			var schema = canReflect_1_18_0_canReflect.getSchema(value);
			if (schema && schema.identity && schema.identity.length) {
				var id = canReflect_1_18_0_canReflect.getIdentity(value, schema);
				if (id != null && id === canReflect_1_18_0_canReflect.getIdentity(newValue, schema)) {
					mergeMap(value, newValue);
					return;
				}
			}
			canReflect_1_18_0_canReflect.setKeyValue(instance, prop, canReflect_1_18_0_canReflect.new(value.constructor, newValue));
		} else {
			canReflect_1_18_0_canReflect.setKeyValue(instance, prop, newValue);
		}
	});
	canReflect_1_18_0_canReflect.eachKey(data, function(value, prop) {
		canReflect_1_18_0_canReflect.setKeyValue(instance, prop, value);
	});
}

function mergeList(list$$1, data) {
	var ItemType, itemSchema;
	var listSchema = canReflect_1_18_0_canReflect.getSchema(list$$1);
	if (listSchema) {
		ItemType = listSchema.values;
	}

	if (ItemType) {
		itemSchema = canReflect_1_18_0_canReflect.getSchema(ItemType);
	}
	if (!itemSchema && canReflect_1_18_0_canReflect.size(list$$1) > 0) {
		itemSchema = canReflect_1_18_0_canReflect.getSchema(canReflect_1_18_0_canReflect.getKeyValue(list$$1, 0));
	}

	var identity;
	if(itemSchema && itemSchema.identity && itemSchema.identity.length) {
		identity = function(a, b) {
		   var aId = canReflect_1_18_0_canReflect.getIdentity(a, itemSchema),
			   bId = canReflect_1_18_0_canReflect.getIdentity(b, itemSchema);
		   var eq = aId === bId;
		   if (eq) {
	...
	var hydrate = ItemType ? canReflect_1_18_0_canReflect.new.bind(canReflect_1_18_0_canReflect, ItemType) : function(v) {
		return v;
	};
...
var canDiff_1_5_0_canDiff = canNamespace_1_0_0_canNamespace.diff = diff;
...
var onKeyValueSymbol$6 = canSymbol_1_6_5_canSymbol.for("can.onKeyValue");
var offKeyValueSymbol$4 = canSymbol_1_6_5_canSymbol.for("can.offKeyValue");
...
function withKey(obj, key, fn) {
	...
	if (isFunction$2(obj[onKeyValueSymbol$6])) {
		canReflect_1_18_0_canReflect.onKeyValue(obj, key, noop$2);
	}

	result = fn(obj, key);

	if (isFunction$2(obj[offKeyValueSymbol$4])) {
		canReflect_1_18_0_canReflect.offKeyValue(obj, key, noop$2);
	}
...
function withoutKey(obj, fn) {
	...
	if (isFunction$2(obj[onValueSymbol$5])) {
		canReflect_1_18_0_canReflect.onValue(obj, noop$2);
	}

	result = fn(obj);

	if (isFunction$2(obj[offValueSymbol$3])) {
		canReflect_1_18_0_canReflect.offValue(obj, noop$2);
	}
...
var makeNode = function makeNode(obj, key) {
	...
	var node = {
		obj: obj,
		name: canReflect_1_18_0_canReflect.getName(obj),
		value: gotKey ? canReflect_1_18_0_canReflect.getKeyValue(obj, key) : canReflect_1_18_0_canReflect.getValue(obj)
	};
...
var getGraph = function getGraph(obj, key) {
	...
	var visitKeyDependencies = function visitKeyDependencies(source, meta, cb) {
		canReflect_1_18_0_canReflect.eachKey(source.keyDependencies || {}, function(keys, obj) {
			canReflect_1_18_0_canReflect.each(keys, function(key) {
				cb(obj, meta, key);
			});
		});
	};
	...
	var visitValueDependencies = function visitValueDependencies(source, meta, cb) {
		canReflect_1_18_0_canReflect.eachIndex(source.valueDependencies || [], function(obj) {
			cb(obj, meta);
		});
	};
	...
	var visit = function visit(obj, meta, key) {
		...
		if (data && data.whatIChange) {
			...
			// kind :: derive | mutate
			canReflect_1_18_0_canReflect.eachKey(data.whatIChange, function(dependencyRecord, kind) {
				nextMeta.kind = kind;
		...
		if (data && data.whatChangesMe) {
			...
			// kind :: derive | mutate
			canReflect_1_18_0_canReflect.eachKey(data.whatChangesMe, function(dependencyRecord, kind) {
				nextMeta.kind = kind;
				visitKeyDependencies(dependencyRecord, nextMeta, visit);
				visitValueDependencies(dependencyRecord, nextMeta, visit);
			});
...
var formatGraph = function formatGraph(graph) {
	...
	var nodesDataSet = graph.nodes.map(function(node) {
		return {
			...
			label:
				canReflect_1_18_0_canReflect.getName(node.obj) +
				(node.key ? "." + node.key : "")
		};
	});
...
var logData = function log(data) {
	...
	if (data.derive.length) {
		...
		canReflect_1_18_0_canReflect.eachIndex(data.derive, log);
		console.groupEnd();
	}

	if (data.mutations.length) {
		console.group("MUTATED BY");
		canReflect_1_18_0_canReflect.eachIndex(data.mutations, log);
		console.groupEnd();
	}

	if (data.twoWay.length) {
		console.group("TWO WAY");
		canReflect_1_18_0_canReflect.eachIndex(data.twoWay, log);
		console.groupEnd();
	}
...
function registerWithDevtools() {
	...
	var devtoolsCanModules = {
		...
		Reflect: canReflect_1_18_0_canReflect,
		...
		mergeDeep: mergeDeep,
		queues: canQueues_1_3_1_canQueues
	};
	 ...
	 */
	{
      ...
      if(process.env.NODE_ENV !== 'production') {
	      Object.defineProperty(controlMethod, "name", {
	      	value: canReflect_1_18_0_canReflect.getName(this) + "["+name+"]",
	      });
	     }
      //!steal-remove-end
      return controlMethod;
			...
			if (options || !paramReplacer.test(methodName)) {
                var controlActionData = function() {
					...
					var name = methodName.replace(paramReplacer, function(matched, key) {
						...
						if (!parent || !(canReflect_1_18_0_canReflect.isObservableLike(parent) && canReflect_1_18_0_canReflect.isMapLike(parent)) && !value) {
                            unableToBind = true;
							return null;
						}
        ...
        if(process.env.NODE_ENV !== 'production') {
		    	Object.defineProperty(controlActionData, "name", {
		      	value: canReflect_1_18_0_canReflect.getName(controlInstance || this.prototype) + "["+methodName+"].actionData",
		      });
	      }
        //!steal-remove-end

				readyCompute = new canObservation_4_2_0_canObservation(controlActionData, this);
          ...
          if(process.env.NODE_ENV !== 'production') {
          	Object.defineProperty(handler, "name", {
            	value: canReflect_1_18_0_canReflect.getName(controlInstance) + "["+methodName+"].handler",
            });
          }
					//!steal-remove-end


					canReflect_1_18_0_canReflect.onValue(readyCompute, handler, "mutate");
          //!steal-remove-start
          if(process.env.NODE_ENV !== 'production') {
	          if(unableToBind) {
		...
		setup: function (element, options) {
			...
			if (canReflect_1_18_0_canReflect.isObservableLike(options) && canReflect_1_18_0_canReflect.isMapLike(options)) {
		...
		on: function (el, selector, eventName, func) {
			if (!el) {
				...
				var removalDisposal = canDomMutate_2_0_7_canDomMutate.onNodeDisconnected(element, function () {
		...
		off: function () {
			...
			if( bindings ) {
				...
				canReflect_1_18_0_canReflect.eachKey(bindings.control || {}, function (value) {
					value(el);
				});
				canReflect_1_18_0_canReflect.eachKey(bindings.readyComputes || {}, function(value) {
					canReflect_1_18_0_canReflect.offValue(value.compute, value.handler, "mutate");
				});
			}
			// Adds bindings.
...
var canControl_5_0_0_canControl = canNamespace_1_0_0_canNamespace.Control = Control;

// Data Models
// Observables

canStache_5_0_0_canStache.addHelper("plusOne", num => num + 1);

class ComponentTreeNode extends canObservableObject {
	static get props() {
		return {
			selected: Boolean,
			tagName: String,
			id: { type: Number, identity: true },
			children: canType_1_1_2_canType.late(() => canType_1_1_2_canType.convert(ComponentTreeList))
		};
	}
}

/* jshint -W003 */
class ComponentTreeList extends canObservableArray {
	static get items() {
		return canType_1_1_2_canType.convert(ComponentTreeNode);
	}
}
/* jshint +W003 */

class ComponentTree extends canStacheElement {
	static get view() {
		return `
			{{< treeNodeTemplate }}
	...
	static get props() {
		return {
			...
			componentTree: {
				type: canType_1_1_2_canType.maybeConvert(ComponentTreeList),
				...
				value({ listenTo, lastSet, resolve }) {
					...
					const resetOnIdChange = () => {
						if (selectedNode) {
							...
							canReflect_1_18_0_canReflect.offKeyValue(selectedNode, "id", resetOnIdChange);
						}
					...
					const setSelectedNode = node => {
						...
						if (selectedNode) {
							canReflect_1_18_0_canReflect.onKeyValue(selectedNode, "id", resetOnIdChange);
						}
					};
					...
					const selectedComponentTreeNode = canValue_1_1_2_canValue.returnedBy(() => {
					...
					const selectedNodeInTree = canValue_1_1_2_canValue.returnedBy(() => {
						return findNode(this.componentTree, node => {
...
class DeepObservableObject extends canObservableObject {
	static get propertyDefaults() {
		return canType_1_1_2_canType.maybeConvert(DeepObservable);
	}
}

class DeepObservableArray extends canObservableArray {
	static get items() {
		return canType_1_1_2_canType.maybeConvert(DeepObservable);
	}
}
...
canReflect_1_18_0_canReflect.assignSymbols(DeepObservable, {
...
var canDeepObservable_1_0_1_canDeepObservable = canNamespace_1_0_0_canNamespace.DeepObservable = canType_1_1_2_canType.maybeConvert(DeepObservable);

class TurningArrow extends canStacheElement {
	static get view() {
		return `
			{{# if(this.down) }}
...
canDomEvents_1_3_11_canDomEvents.addEvent(canEventDomEnter_2_2_1_canEventDomEnter);
...
class EditableSpan extends canStacheElement {
	static get view() {
		return `
			<span
	...
	static get props() {
		return {
			options: canType_1_1_2_canType.convert(canObservableArray),
	...
	connected() {
		const EventsControl = canControl_5_0_0_canControl.extend({
			"li mouseover"(li) {
...
canStache_5_0_0_canStache.addHelper("isObservableArray", val => val instanceof canObservableArray);
canStache_5_0_0_canStache.addHelper("isNumber", val => typeof val === "number");
canStache_5_0_0_canStache.addHelper("isEven", num => num % 2 === 0);

const removeTrailingBracketsOrBraces = str => str.replace(/\[\]|\{\}$/, "");
canStache_5_0_0_canStache.addHelper("removeTrailingBracketsOrBraces", removeTrailingBracketsOrBraces);
...
const getType = val => {
	if (canReflect_1_18_0_canReflect.isObservableLike(val)) {
		if (canReflect_1_18_0_canReflect.isListLike(val)) {
			return "Array";
		} else if (canReflect_1_18_0_canReflect.isMapLike(val)) {
			return "Object";
		}
	}
...
class ParsedJSONNode extends canObservableObject {
	static get props() {
		return {
			key: canType_1_1_2_canType.convert(NumberOrString),
			value: canType_1_1_2_canType.convert(NumberOrString),
			type: String,
			path: String
		};
	}
...
class ParsedJSON extends canObservableArray {
	static get items() {
		return canType_1_1_2_canType.convert(ParsedJSONNode);
	}
}
...
const parseKeyValue = ({ key, value, parentPath }) => {
	...
	const mightHaveChildren = canReflect_1_18_0_canReflect.isObservableLike(value) && (canReflect_1_18_0_canReflect.isListLike(value) || canReflect_1_18_0_canReflect.isMapLike(value));
...
class JSONTreeEditor extends canStacheElement {
	static get view() {
		return `
			{{< singleValueTemplate }}
				{{# switch(type) }}
	...
	static get props() {
		return {
			...
			typeNames: {
				type: canType_1_1_2_canType.convert(canObservableObject),
			...
			expandedKeys: {
				value({ listenTo, lastSet, resolve }) {
					let keys = resolve(new canObservableArray());
					...
					listenTo("add-child", (ev, path) => {
						...
						let parent = canKey_1_2_1_canKey.get(this.json, path);

						if (parent instanceof canObservableArray) {
							path = path + ".0";
						}
			...
			json: {
				type: canDeepObservable_1_0_1_canDeepObservable,

				get default() {
					return canReflect_1_18_0_canReflect.new(canDeepObservable_1_0_1_canDeepObservable, {});
				},
				...
				value({ listenTo, lastSet, resolve }) {
					...
					const resetJson = newJson => {
						json = resolve(canReflect_1_18_0_canReflect.new(canDeepObservable_1_0_1_canDeepObservable, newJson));
					};
					...
					listenTo("set-json-path-value", (ev, path, value) => {
						if (value === "" || value.match(wrappedInQuotesRegex)) {
							...
							value = canStringToAny_1_2_1_canStringToAny(value);
						}

						canKey_1_2_1_canKey.set(json, path, value);
					});

					listenTo("delete-json-path", (ev, path) => {
						canKey_1_2_1_canKey.deleteKey(this.json, path);
					});

					listenTo("add-child", (ev, path) => {
						let parent = canKey_1_2_1_canKey.get(json, path);
			...
			get parsedJSON() {
				...
				canReflect_1_18_0_canReflect.each(this.json, (value, key) => {
			...
			displayedKeyValueEditors: {
				value({ listenTo, lastSet, resolve }) {
					let displayedEditors = resolve(new canObservableArray());
...
class KeyValueEditor extends canStacheElement {
	static get view() {
		return `
			<div class="key">
...
class ViewmodelEditor extends canStacheElement {
	static get view() {
		return `
			<div class="header">
				{{# unless(this.tagName) }}
	...
	static get props() {
		return {
			...
			undefineds: {
				type: canType_1_1_2_canType.convert(canObservableArray),

				get default() {
					return new canObservableArray();
				}
			},

			expandedKeys: canType_1_1_2_canType.convert(canObservableArray),
			editorError: String,
			...
			json: {
				type: canDeepObservable_1_0_1_canDeepObservable,
				value({ listenTo, lastSet, resolve }) {
					let json = resolve(canReflect_1_18_0_canReflect.new(canDeepObservable_1_0_1_canDeepObservable, this.serializedViewModelData));
					let jsonPatches = [];

					const setPatches = (newJSON, oldJSON) => {
						const patches = canDiff_1_5_0_canDiff.deep(oldJSON, newJSON);
					...
					const serializedJSON = new canObservation_4_2_0_canObservation(() => json.serialize());
					canReflect_1_18_0_canReflect.onValue(serializedJSON, setPatches);
					...
					listenTo("serializedViewModelData", (ev, vmData) => {
						...
						canReflect_1_18_0_canReflect.offValue(serializedJSON, setPatches);
						canReflect_1_18_0_canReflect.updateDeep(json, newJson);
						canReflect_1_18_0_canReflect.onValue(serializedJSON, setPatches);
					});
					...
					listenTo("undefineds", (ev, undefineds) => {
						...
						let undefinedsObject = undefineds.reduce((obj, key$$1) => {
							obj[key$$1] = undefined;
							return obj;
						}, {});
						// create list of patches to apply to set undefineds
						let undefinedPatches = canDiff_1_5_0_canDiff.deep({}, undefinedsObject);
			...
			jsonEditorPatches: {
				type: canType_1_1_2_canType.Any,
				get(lastSet) {
					if (lastSet) {
						return lastSet;
					}
					return canDiff_1_5_0_canDiff.deep(this.serializedViewModelData, this.json.serialize());
				}
			},
	...
	getPatchedData(destination, patches) {
		...
		patches.forEach(({ type, key: key$$1, value, index, deleteCount, insert }) => {
			switch (type) {
				case "add":
				case "set":
					canKey_1_2_1_canKey.set(patchedData, key$$1, value);
					break;
				case "delete":
					canKey_1_2_1_canKey.deleteKey(patchedData, key$$1);
					break;
				case "splice":
					let arr = canKey_1_2_1_canKey.get(patchedData, key$$1);
	...
	static get propertyDefaults() {
		return canDeepObservable_1_0_1_canDeepObservable;
	}
}

customElements.define("viewmodel-editor", ViewmodelEditor);

class ExpandableSection extends canStacheElement {
	static get view() {
		return `
			<div
	...
	static get props() {
		return {
			...
			sectionTitle: canType_1_1_2_canType.Any,
...
class BreakpointsEditor extends canStacheElement {
	static get view() {
		return `
			<p>
				<input
	...
	static get props() {
		return {
			...
			addBreakpoint: {
				get default() {
					return expression => console.log(`adding breakpoint for ${expression}`);
				}
			},
...
class ComponentsPanel extends canStacheElement {
	static get view() {
		return `
	...
	static get props() {
		return {
			...
			componentTree: canType_1_1_2_canType.convert(canObservableArray),

			selectedNode: canType_1_1_2_canType.convert(canObservableObject),
			componentTreeError: String,

			// viewmodel editor fields
			viewModelData: canType_1_1_2_canType.convert(canObservableObject),

			typeNamesData: canType_1_1_2_canType.convert(canObservableObject),
			messages: canType_1_1_2_canType.convert(canObservableObject),
			undefineds: canType_1_1_2_canType.convert(canObservableArray),
			expandedKeys: canType_1_1_2_canType.convert(canObservableArray),
			viewModelEditorError: String,

			// breakpoints fields
			breakpoints: canType_1_1_2_canType.convert(canObservableArray),

			breakpointsError: String,
			breakpointsExpanded: Boolean,

			//breakpoints DOM fields
			breakpointsTitle: canType_1_1_2_canType.Any,

			viewModelTitle: canType_1_1_2_canType.Any,
...
export { canStacheElement as StacheElement, canObservableObject as ObservableObject, canObservableArray as ObservableArray, canReflect_1_18_0_canReflect as Reflect, canType_1_1_2_canType as type };
