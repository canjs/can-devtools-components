...
canStache_4_17_19_canStache.addHelper("plusOne", num => num + 1);
...
canComponent_4_6_2_canComponent.extend({
	tag: "component-tree",
	ViewModel: {
		error: "string",
			...
			value({ listenTo, lastSet, resolve }) {
				...
				const resetOnIdChange = () => {
					if (selectedNode) {
						...
						canReflect_1_17_11_canReflect.offKeyValue(selectedNode, "id", resetOnIdChange);
					}
				...
				const setSelectedNode = node => {
					...
					if (selectedNode) {
						canReflect_1_17_11_canReflect.onKeyValue(selectedNode, "id", resetOnIdChange);
					}
				};
				...
				const selectedComponentTreeNode = canValue_1_1_1_canValue.returnedBy(() => {
				...
				const selectedNodeInTree = canValue_1_1_1_canValue.returnedBy(() => {
					return findNode(this.componentTree, node => {
...
canStache_4_17_19_canStache.addHelper("isList", isList);
canStache_4_17_19_canStache.addHelper("isNumber", val => typeof val === "number");
canStache_4_17_19_canStache.addHelper("isEven", num => num % 2 === 0);
const removeTrailingBracketsOrBraces = str => str.replace(/\[\]|\{\}$/, "");
canStache_4_17_19_canStache.addHelper("removeTrailingBracketsOrBraces", removeTrailingBracketsOrBraces);
...
const getType = val => {
	if (canReflect_1_17_11_canReflect.isObservableLike(val)) {
		if (canReflect_1_17_11_canReflect.isListLike(val)) {
			return "Array";
		} else if (canReflect_1_17_11_canReflect.isMapLike(val)) {
			return "Object";
		}
	}
...
const parseKeyValue = ({ key, value, parentPath }) => {
	...
	const mightHaveChildren = canReflect_1_17_11_canReflect.isObservableLike(value) && (canReflect_1_17_11_canReflect.isListLike(value) || canReflect_1_17_11_canReflect.isMapLike(value));
...
const JSONTreeEditor = canComponent_4_6_2_canComponent.extend({
	...
	ViewModel: {
		...
		get parsedJSON() {
			...
			canReflect_1_17_11_canReflect.each(this.json, (value, key) => {
...
const KeyValueEditor = canComponent_4_6_2_canComponent.extend({
	...
	ViewModel: {
		...
		json: {
			value({ listenTo, lastSet, resolve }) {
				...
				const setPatches = (newJSON, oldJSON) => {
					const patches = canDiff_1_4_5_canDiff.deep(oldJSON, newJSON);
				...
				const serializedJSON = new canObservation_4_1_3_canObservation(() => json.serialize());
				canReflect_1_17_11_canReflect.onValue(serializedJSON, setPatches);
				...
				listenTo("serializedViewModelData", (ev, vmData) => {
					...
					canReflect_1_17_11_canReflect.offValue(serializedJSON, setPatches);
					canReflect_1_17_11_canReflect.updateDeep(json, newJson);
					canReflect_1_17_11_canReflect.onValue(serializedJSON, setPatches);
				});
				...
				listenTo("undefineds", (ev, undefineds) => {
					...
					let undefinedPatches = canDiff_1_4_5_canDiff.deep({}, undefinedsObject);
		...
		jsonEditorPatches: {
			...
			get(lastSet) {
				...
				return canDiff_1_4_5_canDiff.deep(this.serializedViewModelData, this.json.serialize());
			}
		},
...
var panel = canComponent_4_6_2_canComponent.extend({
...
export { canComponent_4_6_2_canComponent as Component, map$2 as DefineMap, list$1 as DefineList, canReflect_1_17_11_canReflect as Reflect };
