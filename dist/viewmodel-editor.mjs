...
var canViewCallbacks_4_3_2_canViewCallbacks = createCommonjsModule(function (module) {
...
var canAttributeObservable_1_1_7_behaviors = attr;
...
core.attr = function(el, attributeName, compute) {
	function liveUpdateAttr(newVal) {
		canQueues_1_2_1_canQueues.domUIQueue.enqueue(canAttributeObservable_1_1_7_behaviors.set, canAttributeObservable_1_1_7_behaviors, [el, attributeName, newVal]);
	}
	//!steal-remove-start
	if(process.env.NODE_ENV !== 'production') {
	...
	canAttributeObservable_1_1_7_behaviors.set(el, attributeName, canReflect_1_17_7_canReflect.getValue(compute));
};
	...
	function liveAttrsUpdate(newVal) {
		...
		for (name in newAttrs) {
			...
			if (newValue !== oldValue) {
				...
				var callback = canViewCallbacks_4_3_2_canViewCallbacks.attr(name);
				if (callback) {
					callback(el, {
						attributeName: name,
						scope: scope,
...
if(!canViewCallbacks_4_3_2_canViewCallbacks.tag("content")) {
	// This was moved from the legacy view/scanner.js to here.
	// This makes sure content elements will be able to have a callback.
	canViewCallbacks_4_3_2_canViewCallbacks.tag("content", function(el, tagData) {
...
function stache (filename, template) {
	...
	canViewParser_4_1_2_canViewParser(template, {
		...
		start: function(tagName, unary, lineNo){
			...
			var isCustomTag =  canViewCallbacks_4_3_2_canViewCallbacks.tag(tagName);
			...
			if(unary){
				...
				if(isCustomTag) {
					...
					addAttributesCallback(state.node, function(scope, parentNodeList){
						...
						//!steal-remove-end
						canViewCallbacks_4_3_2_canViewCallbacks.tagHandler(this,tagName, {
							scope: scope,
							subtemplate: null,
							templateType: "stache",
		...
		attrEnd: function(attrName, lineNo){
			...
			if(state.node.section) {
				...
				var attrCallback = canViewCallbacks_4_3_2_canViewCallbacks.attr(attrName);
...
stache.addBindings = canViewCallbacks_4_3_2_canViewCallbacks.attrs;

var canStache_4_15_9_canStache = canNamespace_1_0_0_canNamespace.stache = stache;
...
var canAttributeObservable_1_1_7_event = canEvent;
...
var canAttributeObservable_1_1_7_getEventName = function getEventName(el, prop, bindingData) {
	var event = "change";
	...
	if (canAttributeObservable_1_1_7_behaviors.findSpecialListener(prop)) {
		event = prop;
	}
...
function AttributeObservable(el, prop, bindingData, event) {
	...
	this.event = event || canAttributeObservable_1_1_7_getEventName(el, prop, bindingData);
	this.handler = this.handler.bind(this);
...
canAssign_1_3_1_canAssign(AttributeObservable.prototype, {
	...
	get: function get() {
		...
		var value = canAttributeObservable_1_1_7_behaviors.get(this.el, this.prop);
		if (typeof value === 'function') {
	...
	set: function set(newVal) {
		var setterDispatchedEvents = canAttributeObservable_1_1_7_behaviors.setAttrOrProp(this.el, this.prop, newVal);
	...
	handler: function handler(newVal, event) {
		var old = this._value;
		var queuesArgs = [];
		this._value = canAttributeObservable_1_1_7_behaviors.get(this.el, this.prop);
	...
	onBound: function onBound() {
		...
		observable._handler = function(event) {
			observable.handler(canAttributeObservable_1_1_7_behaviors.get(observable.el, observable.prop), event);
		};

		if (observable.event === internalRadioChangeEventType) {
			canAttributeObservable_1_1_7_event.on.call(observable.el, "change", observable._handler);
		}

		var specialBinding = canAttributeObservable_1_1_7_behaviors.findSpecialListener(observable.prop);
		if (specialBinding) {
			observable._specialDisposal = specialBinding.call(observable.el, observable.prop, observable._handler, canUtilAEL);
		}

		canAttributeObservable_1_1_7_event.on.call(observable.el, observable.event, observable._handler);
	...
	onUnbound: function onUnbound() {
		...
		if (observable.event === internalRadioChangeEventType) {
			canAttributeObservable_1_1_7_event.off.call(observable.el, "change", observable._handler);
		}
		...
		canAttributeObservable_1_1_7_event.off.call(observable.el, observable.event, observable._handler);
	},
...
var canAttributeObservable_1_1_7_canAttributeObservable = AttributeObservable;
...
var behaviors = {
	...
	event: function(el, data) {
		...
		var unbindEvent = function() {
			canAttributeObservable_1_1_7_event.off.call(bindingContext, event, handler);
		...
		canAttributeObservable_1_1_7_event.on.call(bindingContext, event, handler);
...
var getObservableFrom = {
	...
	attribute: function(el, scope, prop, bindingData, mustBeGettable, stickyCompute, event, bindingInfo) {
		return new canAttributeObservable_1_1_7_canAttributeObservable(el, prop, bindingData, event);
	}
};
...
canViewCallbacks_4_3_2_canViewCallbacks.attrs(canStacheBindings);
...
canStache_4_15_9_canStache.addBindings(canStacheBindings_4_6_4_canStacheBindings);
		...
		setup: function() {
			...
			if (Component) {
				...
				if (typeof this.renderer === "string") {
					...
					this.renderer = canStache_4_15_9_canStache(viewName, this.renderer);
				}

				// Register this component to be created when its `tag` is found.
				canViewCallbacks_4_3_2_canViewCallbacks.tag(this.prototype.tag, function(el, tagData) {
		...
		setup: function(el, componentTagData) {
			...
			if (componentContent !== undefined) {
				...
				if (typeof componentContent === "function") {
					...
					componentTagData.subtemplate = canStache_4_15_9_canStache(componentContent);
				}
			}
			...
			if (componentTemplates !== undefined) {
				canReflect_1_17_7_canReflect.eachKey(componentTemplates, function(template, name) {
					...
					if (typeof template === "string") {
						...
						componentTemplates[name] = canStache_4_15_9_canStache(debugName, template);
					}
				});
			}
...
var canDebug_2_0_5_canDebug = function() {
...
canStache_4_15_9_canStache.addHelper("isList", isList);
canStache_4_15_9_canStache.addHelper("isNumber", val => typeof val === "number");
canStache_4_15_9_canStache.addHelper("isEven", num => num % 2 === 0);
const removeTrailingBracketsOrBraces = str => str.replace(/\[\]|\{\}$/, "");
canStache_4_15_9_canStache.addHelper("removeTrailingBracketsOrBraces", removeTrailingBracketsOrBraces);
...
const parseKeyValue = ({ key, value, parentPath, getTypeName }) => {
	...
	if (mightHaveChildren) {
		...
		value.forEach((childValue, childKey) => {
			parsedValue.push(parseKeyValue({ key: childKey, value: childValue, parentPath: path, getTypeName: getTypeName }));
		});
	} else {
		parsedValue = value;
	}
	...
	ViewModel: {
		...
		get parsedJSON() {
			...
			canReflect_1_17_7_canReflect.each(this.json, (value, key) => {
				parsed.push(parseKeyValue({ key, value, getTypeName }));
			});
...
var viewmodelEditor = canComponent_4_4_9_canComponent.extend({
	...
	ViewModel: {
		...
		typeNamesData: { Type: map$2, Default: map$2 },
